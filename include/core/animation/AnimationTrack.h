...
public:
  /**
    ...
    */
  AnimationTrack(std::initializer_list<Keyframe<Value>> keyframes,
  ...
  /**
    ...
    *   duplicate frame numbers.
    */
  explicit AnimationTrack(std::vector<Keyframe<Value>> keyframes,
                          interpolation::InterpolationMode interpolationMode = interpolation::InterpolationMode::Linear)
    : m_keyframes(std::move(keyframes)), m_interpolationMode(interpolationMode) {
  ...
  [[nodiscard]] Value sample(int frame) const {
    ...
    const auto next = std::upper_bound(m_keyframes.begin(), m_keyframes.end(), frame,
                                       [](int value, const auto& keyframe) {
