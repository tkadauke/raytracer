...
public:
  inline explicit InvalidMeshFaceException(const std::string& message, const std::string& file,
                                           int line)
...
public:
  ...
  public:
    ...
    inline bool operator!=(const TriangleIterator& other) const {
      return &m_mesh != &other.m_mesh || m_faceIndex != other.m_faceIndex ||
  ...
  inline void addFace(const Face& face) {
    if (face.size() < 3) {
      throw InvalidMeshFaceException(
