...
- **Software rasterizer.** Scanline + Z-buffer; runs on CPU; no GL dependency. Great for headless preview when the GL stack is unavailable. The educational version implements the textbook pipeline end-to-end: clipping (Sutherland-Hodgman), perspective-correct attribute interpolation, depth and stencil, vertex/fragment shading hooks. Current state ships an edge-function rasterizer (Pineda 1988) with homogeneous clip-space Sutherland-Hodgman against the near plane and viewport planes before perspective divide, configurable depth/stencil state, tiny vertex/fragment shader hooks, perspective-correct depth + normal + worldPos + UV interpolation, Lambertian shading, per-primitive material albedo sampled with the interpolated hit context, switchable `Both` / `Back` / `Front` face culling that defaults to two-sided, a final scissor-rect framebuffer clip in `core::rasterizeTriangle`, opt-in 2x/4x/8x MSAA with per-sample colour/depth/stencil buffers resolved into the float framebuffer, opt-in directional-light shadow maps with PCF filtering, and an opt-in tile-parallel `QThreadPool` path (`queueSize > 1`) that keeps default renders on the faster streaming single-tile path until tile binning wins on measured scenes. Follow-up work:
  ...
  - **Tile-parallel performance follow-up** — the opt-in tiled path is correct but currently slower on measured scenes. Re-measured after MSAA/shadow/FXAA work in May 2026: 640x480 materials 4x MSAA still measured `66.206 ms` single-tile vs `201.243 ms` tiled, and 1920x1080 offscreen-floor 4x MSAA measured `518.987 ms` single-tile vs `1565.575 ms` tiled. Keep it behind `queueSize > 1` until binning, work partitioning, and per-tile setup are cheap enough to beat the streaming single-tile path; track every behavior/performance change with `rendercli` measurements in the changelog.
  ...
  - **Rasterized shadows** — ~~add a shadow-map renderer on top of the depth/stencil path: directional-light shadow maps first~~ ✅ **Done.** `Rasterizer` now has opt-in directional-light shadow maps with `rendercli` and GeneratedRayTracer controls, PCF filtering, rendered comparison sweeps, and an interactive light-depth-map widget. Next add PCSS and cascades for comparison with the raytracer's shadow rays.
