...
- **OpenGL viewport / graph raster backend.** ~~First OpenGL raster executor over graph raster pass state, offscreen context probing, VBO/IBO mesh upload, shader-backed color/depth beauty output, rendercli/Modeler backend selection, MSAA/scissor/culling/write-mask state, texture sampling, directional shadow maps, and CPU readback through explicit graph nodes.~~ ✅ **Done.** The remaining viewport-class work is ~~GPU resource residency scheduling and inspection across passes~~ ✅ **Partially done.** Graph compilation now marks OpenGL producer outputs as GPU-domain until explicit readback boundaries, OpenGL raster outputs publish graph-owned resident attachments, explicit readback nodes copy those attachments back to CPU buffers, and Modeler/rendercli traces expose the residency decision. Remaining work is attachment-load support between compatible GPU passes, gizmo/editor overlay integration, and avoiding CPU readback between compatible GPU passes.
