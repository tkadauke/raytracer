...
- ~~`WireframeEngine`~~ ✅ **Done** (V1, single-threaded, no HLR). Edge projection from `Mesh` via `Camera::projectPoint` (PinholeCamera + inheritors) and Bresenham rasterization. `Primitive::tessellate(lod)` per primitive walked; every face edge drawn. CSG operations show empty (their tessellate returns empty meshes); fish-eye and other non-pinhole cameras render empty (no closed-form `projectPoint` inverse). V2 wins queued: hidden-line removal, near-plane line clipping, edge-parallel threading.
...
- **Software rasterizer.** Scanline + Z-buffer; runs on CPU; no GL dependency. Great for headless preview when the GL stack is unavailable. The educational version implements the textbook pipeline end-to-end: clipping (Sutherland-Hodgman), perspective-correct attribute interpolation, depth and stencil, vertex/fragment shading hooks. Current state ships an edge-function rasterizer (Pineda 1988) with homogeneous clip-space Sutherland-Hodgman against the near plane and viewport planes before perspective divide, configurable depth/stencil state, tiny vertex/fragment shader hooks, perspective-correct depth + normal + worldPos + UV interpolation, Lambertian shading, per-primitive material albedo sampled with the interpolated hit context, switchable `Both` / `Back` / `Front` face culling that defaults to two-sided, a final scissor-rect framebuffer clip in `core::rasterizeTriangle`, opt-in 2x/4x/8x MSAA with per-sample color/depth/stencil buffers resolved into the float framebuffer, opt-in directional-light shadow maps with PCF/PCSS filtering and texel-grid-stabilized 1-4 cascades, and an opt-in tile-parallel `QThreadPool` path (`queueSize > 1`) that stays caller-selected until a scene-aware default policy exists. Follow-up work:
  - **Far-plane clip policy** — cameras do not expose a render-configurable far plane yet, so the homogeneous clipper currently covers near + viewport planes only. Add far-depth semantics deliberately before clipping against the far plane, and make the policy explicit for both raytracer parity and rasterizer teaching.
  - **Geometric polygon-clipping preprocessing** — replace the scissor-rect bbox-scan in `core::rasterizeTriangle` with a Sutherland-Hodgman clip of the input triangle against the four viewport edges, producing an on-screen polygon (3-7 vertices) which gets fan-triangulated and rasterized. This is the 2D teaching counterpart to the current homogeneous clipper: "here's the canonical screen-space polygon clipper, here's the unified clip-space variant." Real GPUs prefer the scissor-rect we have today (one bbox-scan vs N rasterizer calls) so this is purely a teaching artefact, not a perf win.
  ...
  - **Texture/material input expansion** — UV-driven albedo sampling exists; next expand the material input path toward image textures, tangent frames, normal maps, bump/displacement, MIP-mapping, and eventually material-graph inputs shared with the ray/path tracers.
  - **Tile-parallel performance follow-up** — the opt-in tiled path is correct and now wins on screen-heavy materials/offscreen scenes after tile-local MSAA storage, but dense tessellation still loses badly. Re-measured in May 2026: 640x480 materials 4x MSAA measured `59.718 ms` single-tile vs `20.236 ms` tiled, 1920x1080 offscreen-floor 4x MSAA measured `436.677 ms` single-tile vs `95.671 ms` tiled, and 640x480 dense-sphere 1x measured `1841.772 ms` single-tile vs `4028.655 ms` tiled. Keep it behind `queueSize > 1` until a scene-aware default policy can choose safely; track every behavior/performance change with `rendercli` measurements in the changelog.
  - **Rasterizer anti-aliasing follow-up** — MSAA now exists for coverage/depth and resolve. Next, decide whether to add centroid/per-fragment shading for cheaper MSAA previews, then move into the shared sample-pattern/filter API, post-process AA, TAA, alpha-to-coverage, and conservative rasterization from §4.1.b.
  ...
  - **Rendered-doc engine parity backlog** — keep class-level rendered docs honest about which engines can support each scene. Remaining deferred coverage is explicit: `Ring`, `Union`, `Intersection`, `Difference`, `MinkowskiSum`, `ConvexHull`, and the CSG-heavy `ScriptedSurface` scenes stay raytracer-only until mesh booleans / useful scripted tessellation exist; `FishEyeCamera`, `SphericalCamera`, `EquirectangularCamera`, `ThinLensCamera`, and `TiltShiftCamera` stay raytracer-only until they expose forward projection APIs for Rasterizer and Wireframe; material docs skip Wireframe because it ignores material shading.
  - **Rasterized shadows** — ~~add a shadow-map renderer on top of the depth/stencil path: directional-light shadow maps first~~ ✅ **Done.** `Rasterizer` now has opt-in directional-light shadow maps with `rendercli`, GeneratedRayTracer render-dialog controls, a GeneratedRayTracer live-preview shadow toggle, PCF/PCSS filtering, texel-grid-stabilized 1-4 cascades, rendered comparison sweeps, an interactive light-depth-map widget, and an interactive cascade-split widget. A live-preview split diagnostic remains follow-up work if the docs widget is not enough for judging cascade placement.
...
- **Rasterizer MSAA** — the software rasterizer now has fixed 2x/4x/8x subpixel coverage with independent per-sample colour/depth/stencil buffers and a resolve into the float framebuffer. Current V1 shades covered samples directly for correctness; the next quality/performance decision is whether to add centroid/per-fragment shading as the cheaper preview mode.
- **Post-process AA** — ~~FXAA for the software rasterizer preview path~~ ✅ **Done.** `render::postprocess::applyFxaa` now runs as an opt-in `Rasterizer::PostProcessAA::FXAA` pass, exposed through `rendercli --post_aa fxaa` and the GeneratedRayTracer render dialog. SMAA remains a sharper image-space follow-up for the future OpenGL/WebGL preview path, because editor responsiveness sometimes matters more than ground-truth edge coverage.
...
- ✅ **World scene timeline loading and evaluation.** Editable scenes now preserve a top-level JSON `animation` block and evaluate id-targeted direct `Q_PROPERTY` tracks through `world::Timeline` / `world::AnimationTrack`; `rendercli` frame and image-sequence output still TODO.
