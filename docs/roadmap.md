...
- ~~`WireframeEngine`~~ ✅ **Done** (single-threaded, no HLR). Edge projection from `Mesh` via `Camera::projectPoint` (PinholeCamera + inheritors), near-plane edge clipping, and Bresenham rasterization. `Primitive::tessellate(lod)` per primitive walked; every face edge drawn. CSG operations show empty (their tessellate returns empty meshes); fish-eye and other non-pinhole cameras render empty (no closed-form `projectPoint` inverse). V2 wins queued: hidden-line removal and edge-parallel threading.
