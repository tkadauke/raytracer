...
- ~~**Whitted-only integrator.** Hard-coded into `Raytracer::rayColor`.~~ ✅ **Partially retired.** The existing recursive policy now lives in `render::WhittedIntegrator`, with `Raytracer::rayColor` delegating through the `Integrator` boundary. `render::PathTracingIntegrator` now ships as an iterative Monte Carlo policy with BSDF sampling, next-event light sampling, MIS helpers, Russian roulette, and scalar/batched entry points selectable through ray-family graph state; `engine::wavefront::WavefrontRaytracer` can drive both Whitted and path-tracing batches depth-major. Remaining transport work is production path-tracing breadth: non-delta area/environment/mesh lights, soft shadows, material/light queue specialization, denoising integration, and making non-Whitted policy defaults intentional rather than experimental.
...
- **Bump and cross-engine normal mapping remain incomplete.** Raster previews now support tangent-space normal maps from triangle UVs, but ray/path-tracing still needs `HitPoint` tangent frames plus a shared shading-normal pass before BSDF eval. Bump and displacement mapping are still missing.
...
- ~~**`Light` is not first-class enough.** Lights exist but aren't sampled for soft shadows or MIS — no `Light::sample(point) → (direction, pdf, intensity)`.~~ ✅ **Partially done.** Runtime lights now expose `Light::sample(point)`, `Light::pdf(point, direction)`, delta-light metadata, `emission()`, and `power()` for Epic #358. `render::RectangularAreaLight` (with a `world::RectangularAreaLight` editable wrapper) ships as the first non-delta emitter — `isDelta()` returns `false`, and both `WhittedIntegrator` and `PathTracingIntegrator` consume it through the generic `Light::sample` contract, giving soft shadows; see `docs/markdown/ray-rendering/lights-and-shading.md`. Disk/sphere/cylinder area shapes, mesh emitters, and HDRI environment lights remain future work.
...
- ~~**No time dimension** on the scene graph. Blocks animation.~~ ✅ **Partially done.** Native scene JSON now has a top-level `animation` timeline, world-side keyframe tracks, frame evaluation, `rendercli --frame`, `rendercli --animation`, read-only Modeler timeline preview, and render-side continuous track compilation plus shutter-time sampling for eligible transform/camera/light/material tracks. Richer editors, interpolation modes, and non-transform animation families remain §4.7 follow-up work.
- ~~**`BSDF` is not separated from `Material::shade`.** Today shading is `Material::shade(raytracer, ray, hit, state) → Color` — tightly coupled to the recursive raytracer's call style.~~ ✅ **Partially done.** `render::BSDF` exists with `eval`, `sample`, `pdf`, `reflectance`, explicit delta metadata, and lobe flags; `BRDF` / `BTDF` adapt the legacy lobes to that interface, and representative diffuse/glossy/delta lobes now have sample/PDF contract coverage. Built-in materials expose path-transport hooks consumed by `PathTracingIntegrator` and wavefront batches. `Material::shade` is still the Whitted compatibility layer, richer material-owned BSDF composition remains future work, and recursion ownership cleanup remains future work.
...
- **Sampler distribution properties** — ~~verify jittered sampler actually jitters~~ ✅ **Done.** `JitteredSamplerTest.cpp` now pins one sample per stratum and differing jittered sets; `RegularSamplerTest.cpp` (`ShouldBeEquallySpacedHorizontally` / `ShouldBeEquallySpacedVertically`) already pins regular-sampler stratification. `RandomSamplerTest.cpp` covers seeded determinism and seed-to-seed divergence but not statistical uniformity of the sample values themselves — that check remains open.
- **Factory methods** — `CameraFactory`, `SamplerFactory`, `ViewPlaneFactory` still need focused factory suites. Existing widget tests exercise camera/view-plane identifier enumeration and `HaltonSamplerTest.cpp` pins Halton's sampler-factory registration, but the built-in camera, sampler, and view-plane registries are not covered exhaustively.

Estimated effort: ~1-2 days for the remaining R0 gaps: nested-medium behavior, the `PlaneTest.cpp::ShouldReturnBoundingBox` stub, random-sampler statistical uniformity, and factory-focused tests.
...
- ✅ `SoftwareRasterEngine` — shipped as `engine::raster::Rasterizer` (`include/engine/raster/Rasterizer.h`, a `render::RenderEngine` subclass); same "kept its own name" precedent as `RaytracerEngine`/`Raytracer` below.
- ✅ `OpenGLEngine` — shipped as `engine::raster::OpenGLRasterizer` (`include/engine/raster/OpenGLRasterizer.h`, also a `render::RenderEngine` subclass); see `OpenGLRasterizerTest.cpp` / `OpenGLRasterizerParityTest.cpp`.
...
~~Introduce a `BSDF` interface with `eval(wi, wo) → spectrum`, `sample(wi) → (wo, pdf)`, `pdf(wi, wo) → density`.~~ ✅ **Partially done.** `render::BSDF` now exists and the legacy `BRDF` / `BTDF` hierarchy inherits from it, with lobe flags, explicit delta metadata, caller-owned 2D sampling, and adapters for `eval`, `sample`, `pdf`, and `reflectance`. Lambertian diffuse and Phong glossy lobes now provide analytic sample/PDF contracts, and perfect reflection/transmission expose delta contracts. `PathTracingIntegrator` owns iterative path recursion over the material path-transport hooks. Remaining work: make `Material::shade` a thin Whitted compatibility layer over richer material-owned BSDF composition.
...
- **Software rasterizer.** Scanline + Z-buffer; runs on CPU; no GL dependency. Great for headless preview when the GL stack is unavailable. The educational version implements the textbook pipeline end-to-end: clipping (Sutherland-Hodgman), perspective-correct attribute interpolation, depth and stencil, vertex/fragment shading hooks. Current state ships an edge-function rasterizer (Pineda 1988) with homogeneous clip-space Sutherland-Hodgman against configurable near/far clip depths and viewport planes before perspective divide, configurable depth/stencil/alpha/blend/load/store/viewport/scissor state, tiny vertex/fragment shader hooks, perspective-correct depth + normal + worldPos + UV interpolation, local Matte/Phong material preview with direct UV/checker/image texture paths, mipmapped image textures, tangent-space normal maps, recursive-material fallback diagnostics, material-sidedness-driven default culling, diagnostic depth/normal/id/stencil outputs, opt-in 2x/4x/8x MSAA with per-sample or per-fragment shading, FXAA/SMAA/TAA post-process modes, directional-light shadow maps with PCF/PCSS filtering and texel-grid-stabilized 1-4 cascades, graph-visible visibility/culling resources, optional depth-prepass heuristics, and scene-aware automatic tile-parallel queue selection that preserves explicit caller overrides. Follow-up work:
  ...
  - ~~**Rasterizer performance diagnostics and depth-prepass wave** — collect per-render metrics, preserve imported winding/sidedness, reduce dense-preview tessellation, order opaque work front-to-back, add conservative tile depth rejection, adapt queue sizes from measured tile behavior, and expose an optional depth prepass.~~ ✅ **Done.** The archived `docs/plans/complete/rasterizer-performance.md` plan records the completed phases, with `docs/perf/raster-baseline-scenes-2026-05-31.md` holding the benchmark closeout.
  - **CPU visibility/culling graph pass** — ~~add graph-visible visibility resources, frustum culling, front-to-back ordering, conservative backface filtering, tile diagnostics, and cached visibility artifacts consumed by CPU raster and OpenGL mesh preparation~~ ✅ **Partially done.** Remaining work is explicit tile-occlusion rejection, importer/source-provenance reliability in the visibility cache, full invalidation rules, and auto-insertion thresholds.
...
- **OpenGL viewport / graph raster backend.** ~~First OpenGL raster executor over graph raster pass state, offscreen context probing, VBO/IBO mesh upload, shader-backed color/depth beauty output, rendercli/Modeler backend selection, MSAA/scissor/culling/write-mask state, texture sampling, directional shadow maps, and CPU readback through explicit graph nodes.~~ ✅ **Done.** The typed OpenGL resident-resource/storage substrate has also started, but the remaining viewport-class work is pass-to-pass GPU residency, attachment-load support for graph execution, gizmo/editor overlay integration, and avoiding CPU readback between compatible GPU passes.
...
- **Path tracer.** Monte Carlo integrator over the same scene graph. Multiple Importance Sampling between BSDF sampling and light sampling. Stratified or Sobol QMC sampling. Adaptive sampling per tile. The implementation should explicitly compare **megakernel**, **wavefront/queue-based**, and **material-sorted** architectures: a simple megakernel is the clearest CPU baseline, while wavefront queues expose GPU divergence, occupancy, compaction, and BSDF/light sorting tradeoffs. ~~SoA ray packets (`Ray4`/`Ray8`), primitive packet entry points, and block-batched BVH traversal for CPU packet traversal.~~ ✅ **Done.** Added aligned packet transport, scalar-fallback packet primitive entry points, shared SIMD packet kernels for core primitives, `BoundingBox::intersects4`, and `BVH::intersectPacket(Ray4/Ray8)` active-mask descent with `BVHPacketBenchmark.cpp` coverage for Epic #141 Phases 4.1 and 4.3. `Ray4` is supported through SSE or NEON where available; `Ray8` remains AVX-only.
  ...
  ~~Initial deterministic CPU/GPU tracing sample generator choice.~~ ✅ **Done.** The first parity stream is documented as a stateless 32-bit PCG hash over explicit sample coordinates in `docs/plans/tracing-execution-backends.md`, with platform RNGs excluded for issue #586.
  ...
  These completed bullets are **path-tracing foundations**, not a finished production path tracer: only the single `RectangularAreaLight` shape ships soft shadows (disk/sphere/cylinder/mesh/HDRI emitters are still TODO), wavefront scheduling still needs explicit material/light/shadow queue specialization, ~~denoising is not wired in~~ ✅ **Done.** `render::Denoiser` / `BoxDenoiser` / `BilateralDenoiser` (`include/render/denoise/`) are wired into `engine::wavefront::WavefrontRaytracer` (`setDenoiser`/`clearDenoiser`/`denoiser()`), selectable via `rendercli --wavefront_denoiser {none,box,bilateral}` and Modeler's render settings, and tested in `WavefrontRaytracerTest.cpp` (landed across `18d04789`, `dac80f06`, `ae76e421`, `c7c2cfa7`) — and the default transport policy remains `render::WhittedIntegrator`.
...
2. Add a minimal `RenderPass` interface with ~~declared reads/writes~~, ~~clear/load/store operations for raster framebuffer state~~, and ~~an executor enum~~. ✅ **Partially done.** `engine::graph::RenderPassNode` declares read/write resources, executor, pass kind, disabled behavior, graph validation/export, disabled default substitution, and color passthrough. `RasterFramebufferState` serializes color/depth/stencil load/store intent into graph-backed raster passes. Remaining work is broader graph execution of non-raster load/store semantics and GPU-resident attachment handoff.
3. Split current monolithic engines into first passes without changing output: ~~graph-backed whole-frame raytracer, rasterizer, and wireframe beauty execution~~ ✅ **Partially done.** `RenderGraphCompiler` emits whole-frame beauty plans and `GraphRenderEngine` executes them through the existing Raytracer/Rasterizer/Wireframe engines. `RenderPassPayloads.cpp` now has named payload classes for raytrace, wavefront, raster, wireframe beauty, tonemap, readback, composite, AOV, shadow, visibility, postprocess AA, wireframe overlay, and curve overlay passes. Remaining work is depth-aware overlay composition and richer selector/history-driven pass families.
...
6. Add planar reflections and ~~raster shadow maps~~ as pass-graph clients. ✅ **Done for raster shadow maps.** `RenderGraphCompiler::compileBeautyPlan` emits a `RenderPassKind::Shadow` node (`id = "raster_preview_shadows"`) producing the `preview_shadow_map` resource the beauty pass consumes (`src/engine/graph/RenderGraphCompiler.cpp` around lines 990-1004). Planar reflections as a pass-graph client remain TODO — the portal/mirror render-to-texture subviews from item 4 above cover portals and flat mirrors specifically, not the general "mark any material as reflective" case this bullet describes.
7. Add ~~raytraced shadow masks~~ and path-traced inset/hero passes once AOV resource sharing is solid. ✅ **Done for raytraced shadow masks.** `RenderGraphCompiler::hybridShadowMaskPass(...)` compiles a raytraced `RenderPassKind::Shadow` node producing a `RenderResourceType::ShadowMask` resource, composited via `hybrid_shadow_composite` and selected through `RenderRasterShadowMode::RayTraced` (`include/engine/graph/RenderEngineOptions.h`). Path-traced inset/hero passes remain TODO — `RenderExecutorKind` has no `PathTracer` value yet, only whole-frame engine selection.
...
- **Tiled, scanline, packet, and streaming execution** — contrast image-space decomposition with ray/path queues and ray packets. ✅ **Partially done.** `WhittedIntegrator::radianceBatch(...)` and `PathTracingIntegrator::radianceBatch(...)` now process active frontiers through Ray8/Ray4 packet intersections when driven by the wavefront engine, with packet-fill, scalar-tail, fallback, and refined-lane metrics exposed in tests and trace output. Direct recursive `Raytracer` camera/sample-loop packetization remains tracked in `docs/plans/whitted-ray-packets.md`.
...
- ~~**HDR/display split** — preserve the `Buffer<Colord>` accumulator for offline output while the display path owns LDR snapshots, tonemap timing, and optional progress overlays.~~ ✅ **Partially done.** Current CPU renderers already keep LDR display snapshots separate from HDR engine output, `render::TracingAccumulationLayout` defines the backend accumulation ABI as separate `rgba32_float` color sums, `uint32` sample counts, optional second-moment sums, and `rgba8_unorm_srgb` resolve output, and CPU reference plus optional Metal and Vulkan clear/add/resolve operations now cover synthetic accumulation parity with diagnostics. Remaining work is integrating GPU-owned accumulation/progressive readback into real tracing render loops and graph resources.
- **Resize/cancel safety** — buffer swaps, render cancellation, and widget resize need a clear ownership protocol so Modeler can change scenes/cameras while long renders are stopping. ~~Cancellation / engine-swap ownership~~ ✅ **Partially done.** `RenderWidget::stop()` is documented and used as the blocking teardown path before destruction, resize, or an engine swap, and `cancelRender()` documents snapshot-detach-and-drain semantics (`include/widgets/RenderWidget.h`); covered by `RenderWidgetTest.cpp` (`ShouldKeepFrontBufferVisibleWhileNextRenderStarts`, `ShouldIgnoreCompletionFromStoppedRenderThread`). Resizing *during* an active render is still explicitly documented as undefined (`setBufferSize`'s docstring, `include/widgets/RenderWidget.h:115-118`) and has no test coverage — that half remains open.
...
- ~~Cylinder~~ ✅ **Done.** `world::Cylinder` (`include/world/objects/Cylinder.h`) composes `render::OpenCylinder` + two end `Disk`s (plus optional `Torus` bevel edges) behind a `ClosedSolidUnion`, with radius/height/bevelRadius `Q_PROPERTY`s, doc images (`scripts/docs/cylinder.rb`), and `ElementFactory` registration. Cone, capsule (cylinder with hemispherical caps) remain TODO.
- Heightfield / terrain.
...
- ~~BVH block-batched packet traversal (Ray4/Ray8 active-mask descent; coherent primary-ray cache-reuse path)~~ ✅ **Done.** `BVH::intersectPacket(Ray4/Ray8)` added with active-mask descent; `BVHPacketBenchmark.cpp` measures coherent vs incoherent throughput; closes Epic #141 Phase 4.3. Wavefront Whitted/path-tracing batches now consume the packet traversal path for active frontiers; direct recursive `Raytracer` packet rendering remains a separate optimization plan.
...
- **Emission materials** — ~~constant~~, textured, and mesh-emitter-backed surfaces, with the same material usable by the Whitted, raster, and path-tracing engines even if only the latter two can exploit every path. ✅ **Partially done.** `render::EmissiveMaterial` ships a constant-radiance emitter (`include/render/materials/EmissiveMaterial.h`, tested in `test/unit/render/materials/EmissiveMaterialTest.cpp`); `render::RectangularAreaLight` uses it for its emitter primitive (`src/render/lights/RectangularAreaLight.cpp:128`) and `PathTracingIntegrator::emittedRadiance` (`src/render/PathTracingIntegrator.cpp`) consumes it generically for any material. Textured and mesh-emitter-backed variants, a `world::` editable wrapper for user-authored emissive surfaces, and raster-engine wiring remain TODO.
...
- Beauty (RGB).
- ~~Depth (Z) and normal.~~ ✅ **Done.** `engine::graph::RenderAOVDefinition` registers `Depth` and `Normal` AOVs (`src/engine/graph/RenderAOV.cpp`), exported as graph-visible resources reachable from `rendercli` and Modeler view-mode selection; see `test/unit/engine/graph/RenderGraphCompilerTest.cpp`.
- Albedo / diffuse / specular separation.
- Direct vs indirect; per-light contribution; per-material-id mask.
- ~~Object-id and material-id (for compositing and selection).~~ ✅ **Done.** `ObjectId` and `MaterialId` AOV definitions ship in `RenderAOV.cpp`, with OpenGL-raster ID-AOV fallback and tracing coverage (`509e8709` "Fallback OpenGL raster ID AOVs to CPU diagnostics", `85052b87` "Trace OpenGL raster ID AOV fallbacks").
- Cryptomatte (for proper anti-aliased ID masks).
- Motion vectors (for temporal denoise and motion blur compositing). *(Note: `RenderResourceType::MotionVector` exists as an internal resource type, used by the rasterizer's own TAA history buffer, but has no `RenderAOVDefinition` — not yet a first-class exported AOV.)*
- ~~World-space position~~, UV. ✅ **Partially done.** `WorldPosition` is a registered `RenderAOVDefinition` (`RenderAOV.cpp`); UV as a first-class AOV output remains TODO.
- Roughness, metallic, emission.
- ~~Sample variance (for adaptive sampling and denoise).~~ ✅ **Done.** `SampleStddev` / `SampleStddevColor` AOVs ship (`afc68fc9` "Expose path tracing sample stddev AOV"); also see the related `HybridVisibility` AOV (`c3e11407` / `1317a80b`) which wasn't on this list but is the same family of graph-visible diagnostic outputs.
...
- **Area** — rectangular, disk, sphere, cylinder emitters with proper sampling (uniform, importance, projected solid angle). ⏳ **Partially done.** `render::RectangularAreaLight` / `world::RectangularAreaLight` ship a one-sided rectangular emitter with uniform-area sampling, solid-angle PDF, `emission()`/`power()`, and an `emitterPrimitive()` for direct visibility; see `test/unit/render/lights/RectangularAreaLightTest.cpp`. Disk, sphere, and cylinder emitter shapes, importance/projected-solid-angle sampling variants, and two-sided emission remain TODO.
...
All require ~~a `Light::sample(shadingPoint) → (wi, pdf, Le)` API for proper integration in MC integrators, plus a `Light::pdf(wi)` for MIS and `Light::power()` / bounded-emission metadata for light-tree construction and many-light sampling~~ ✅ **Done.** Runtime lights now expose the sampling, PDF, delta-light, emission, and power metadata contract for Epic #358, and both `WhittedIntegrator` and `PathTracingIntegrator` already consume that contract generically — `RectangularAreaLight` (see above) is the first non-delta light to exercise it. Remaining work is to add the other non-delta area/environment/mesh emitter shapes.
...
- ~~Object hierarchy / outliner with selection, naming, grouping, parenting.~~ ✅ **Done.** `widgets::world::SceneModel` (`include/widgets/world/SceneModel.h`, a `QAbstractItemModel`) backs a tree-view "Elements" outliner dock in `MainWindow` (`src/modeler/MainWindow.cpp`).
- ~~Property editor for the selected object's transform, material, primitive parameters.~~ ✅ **Done.** `widgets::world::PropertyEditorWidget` (`include/widgets/world/PropertyEditorWidget.h`) auto-builds spinbox/parameter controls from the selected `Element`'s `Q_PROPERTY`s, per the pattern this roadmap cites elsewhere (e.g. §4.4.a's "ThinLensCamera commit" checklist).
- Move / rotate / scale gizmos with snapping (unit, grid, surface, increment).
- Primitive creation toolbar; ~~CSG operation buttons~~. ✅ **Partially done.** `MainWindow` has an "Add Primitive" menu/toolbar (`addBoxAct` / `addSphereAct` / `addCylinderAct` / `addRingAct` / `addTorusAct`, `src/modeler/MainWindow.cpp`). Dedicated CSG operation buttons are not yet in the UI — CSG is currently only reachable through the MCP `csg_union` / `csg_intersect` / `csg_difference` chat tools (§4.6.i).
...
- ✅ **Render-side continuous track compilation.** Eligible transform, camera,
  light, and material tracks now compile into Qt-free render tracks attached to
  runtime objects during world-to-render conversion; frame-baked and step-only
  tracks stay on the existing evaluated-world path. ✅ **Done.** Issue #491
  added rendercli parity/shutter coverage and documented the Modeler preview
  preservation path.
- Interpolation curves: linear, Bezier, ease-in/out, hold. ⏳ **Partially done.** `core::math::interpolation::InterpolationMode` (`include/core/math/interpolation/Interpolation.h`) ships `Step` (hold), `Linear`, and `SmoothStep` (ease-in/out), wired through both `world::AnimationTrack` and `render::AnimationTrack`/`AnimationValue`. Bezier curves (explicit tangent handles) remain TODO.
- ✅ **Time-sampled rendering for motion blur** (multiple time samples per frame within shutter-open). First pass landed in 7c81d11 — `State::timeSample` drawn from `SampleStream::next1D` (dim 1 in the renderer's stream allocation), `world::Surface::velocity` Q_PROPERTY, `Instance` interpolates linear translation. Runtime-continuous instance transform tracks now sample position/rotation/scale/velocity and pinhole-camera pose tracks now sample at `State::animationTime` (`animationFrame + shutter offset`) without rebuilding the world scene per ray. Additional interpolation curves and non-transform animation families still TODO.
...
- ~~**Bilateral filter**~~ — joint domain-and-range Gaussian. The lightweight denoiser. ✅ **Done, as a path-tracer consumer** (this section's own stated graduation trigger — see the §4.11 intro — is met for this one algorithm; it hasn't graduated to `include/cv/`, and the rest of §4.11 is still unimplemented or test-helper-only). `render::BilateralDenoiser` (`include/render/denoise/BilateralDenoiser.h`) implements the joint domain-and-range filter over path-tracer feature buffers, tested in `BilateralDenoiserTest.cpp` (7 cases) and wired into `engine::wavefront::WavefrontRaytracer` (`18d04789` "Add wavefront bilateral denoiser", `dac80f06`, `ae76e421`, `c7c2cfa7`).
