...
> **Scope:** archived follow-up plan for the software rasterizer after the
...
- #168 / #169 / #170 / #171 completed the tiling benchmark, automatic
  scene-aware queue policy, projected tile-coverage rejection, and MSAA scratch
  reuse work.
- #172 / #181 completed grouped-bound raster traversal and the 3D frustum
  clipping teaching widget.
- #173 / #174 / #175 / #176 / #177 completed alpha/source-alpha blending,
  recursive-material fallback diagnostics, image texture filtering/mipmaps,
  tangent-space normal maps, and material-sidedness-driven culling defaults.
- #178 / #179 / #180 completed SMAA, the TAA resource contract, and the
  implemented TAA mode.
- API docs, rasterization textbook chapters, shadow/AA/material rendered-doc
  drivers, and widget tests now describe the implemented behavior. Remaining
  work belongs to the roadmap or the render-graph plan, not to this closed
  rasterizer-v2 issue plan.
...
Status: completed for the Rasterizer v2 material and texture scope. Broader
cross-engine material graph, bump/displacement, and anisotropic-filtering work
stays in `docs/roadmap.md` §4.3.
...
- ✅ image texture filtering and mipmaps, with UV-gradient mip selection in the
  raster material path;
- ✅ tangent-space normal mapping from triangle UV tangent frames;
- ✅ material-sidedness-driven default culling;
- ✅ explicit recursive-material fallback diagnostics for reflective and
  transparent materials.

The rasterizer deliberately still avoids pretending recursive raytracer
features are fully supported: reflective and transparent materials report the
local-preview fallback used by `rendercli --engine raster`.

### 10. Continue post-process AA work

Status: completed for the Rasterizer v2 AA scope. Future neighborhood clamps,
motion-vector policy, TAAU, alpha-to-coverage, conservative rasterization, and
shared cross-engine AOV integration remain in `docs/roadmap.md` §4.1.b and
...
- ✅ add TAA accumulation once the resource contract below is backed by
  rasterizer-owned color/depth history, deterministic jitter, first-frame /
  resize / explicit reset handling, and rendercli/Modeler selection;
...
The later 3D frustum-clipping widget also landed: it shows the render camera's
frustum as a wireframe volume, clips source geometry against that volume, and
lets an independent observer camera orbit the setup.
...
Their remaining open items were migrated here and closed or rehomed:

- Modeler cascade split diagnostic decision: closed; diagnostics stay in the
  docs/widget surface until a broader Modeler debug-overlay framework exists.
- SMAA/TAA follow-up: closed for Rasterizer v2; richer temporal/AOV ownership
  moved to the render-graph plan.
- tile-parallel default-policy retry: closed with the scene-aware automatic
  queue policy; further CPU/GPU tiling work moved back to the roadmap.
- frustum/spatial culling integration: grouped-bound traversal landed; broader
  spatial-index culling remains in the roadmap's acceleration work.
