...
| Backend frontier residency and payload size | `intersectionBackendClosestHitFrontierResidency`, `intersectionBackendAnyHitFrontierResidency`, closest-hit/any-hit `PackedRayBytes`, `HostPackedRayBytes`, `HostQueryBytes`, and `StateHandleBytes` | `closest_hit_frontier_residency`, `any_hit_frontier_residency`, closest-hit/any-hit `*_frontier_packed_ray_bytes`, `*_frontier_host_packed_ray_bytes`, `*_frontier_host_query_bytes`, and `*_frontier_state_handle_bytes` | Pass summary reports residency and frontier payload byte totals. |
...
| Frontier compaction | `frontierCompaction*`, `frontierCompactionExecutionPath`, `frontierCompactionPathStateResidency`, compaction candidate keys, largest compaction candidate keys | `frontier_compaction_*`, `frontier_compaction_candidate_*`, `frontier_largest_compaction_candidate_*` | Pass summary reports executed compaction, retained-index bytes, path-state residency, host path-state bytes, candidates, and largest candidate depth. |
