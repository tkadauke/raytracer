...
8. Add rendercli graph inspection, graph-only compilation, graph JSON input,
   ...
   depth, normal, and object-id AOV views, and validate the manipulated plan.
...
view mode can now compile graph-visible `depth_aov`, `normal_aov`, and
`object_id_aov` resources with visualization passes, and rendercli accepts
`--render_graph_view depth`, `--render_graph_view normal`, and
`--render_graph_view object_id`. World-position, material-id, motion vector,
and multi-AOV export files remain TODO.
