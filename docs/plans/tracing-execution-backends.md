...
- The Modeler final render dialog and rendercli graph options expose the GPU
  sample stream as an explicit Path Tracer choice. Full-GPU compiled path-loop
  eligibility now treats ordinary sampler-backed Path Tracer requests as
  CPU/hybrid fallback cases instead of silently replacing the chosen sampler
  with the GPU stream.
