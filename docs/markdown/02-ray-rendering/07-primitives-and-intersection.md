...
- `tessellate(int lod)` — produces a triangle mesh approximation
  ...
  ([chapter 17](../04-rasterization/17-tessellation.md)) needs
...
([chapter 8](08-materials-and-brdfs.md)).
...
mesh-triangle variants in §7.8 carry per-vertex data and
interpolate.

## 7.6 Disk and open cylinder
...
formula from §7.2 applies, with the $y$-coordinate consistency
...
## 7.7 Torus: a quartic root problem
...
point using the barycentric coordinates from §7.5. The
