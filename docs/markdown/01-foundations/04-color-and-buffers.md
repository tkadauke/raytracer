...
## 4.1 `Colord`: three doubles, no alpha
...
the renderer is in HDR mode (§4.4). The class doesn't clamp on
...
## 4.2 The `color_model_conversions` widget
...
([chapter 12](../02-ray-rendering/12-tone-mapping.md)) — even
...
## 4.5 `Buffer<T>`: a fixed-size 2D array
...
Scene + Camera
      │
      │  (chapter 5: the Whitted pipeline)
      ▼
Buffer<Colord>            ← HDR float framebuffer
      │
      │  (chapter 12: tonemap operator)
      ▼
