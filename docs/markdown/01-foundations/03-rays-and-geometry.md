...
introduces the `Ray` type that all of Volume II's intersection code
...
[chapter 2 §2.4](02-matrices-and-transforms.md#2-4-point-vs-direction-vs-normal).
...
- `transform(pointMatrix, normalMatrix)` returns a copy with the
  ...
  from [chapter 2 §2.6](02-matrices-and-transforms.md#2-6-the-four-matrix-dance-instance).
...
## 3.5 `HitPointInterval`: in / out / in / out
...
That's [chapter 5](../02-ray-rendering/05-the-whitted-pipeline.md)'s
...
1. Write `Ray::reflected(const HitPoint&)` that returns a new ray
   ...
   formula from the chapter 1 exercises.) Call `epsilonShifted()`
   on the result. Why?
...
- Vector vocabulary: [1. Numbers and vectors](01-numbers-and-vectors.md)
- Matrix-on-`HitPoint`: [2. Matrices and transforms §2.6](02-matrices-and-transforms.md#2-6-the-four-matrix-dance-instance)
