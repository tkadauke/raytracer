...
actually keep that state on device. The compaction candidate diagnostics apply
the same split to inactive paths: packed-ray bytes estimate the intersection
payload that could be compacted, while state-handle bytes estimate the
