...
- **Beauty pass** — the main shaded color render before
...
- **MIS** — Multiple Importance Sampling. Combines samples drawn
  from different distributions, such as BSDF samples and light
  samples, with weights that reduce variance without double-counting
  the same integral.
  [Materials and BRDFs: MIS helper contracts](../ray-rendering/materials-and-brdfs.md#mis-helper-contracts).
