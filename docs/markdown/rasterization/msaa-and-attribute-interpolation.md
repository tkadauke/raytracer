...
produced in [The rasterization pipeline](the-rasterization-pipeline.md), but
in different ways.
...
## <a id="screen-space-barycentric-weights-aren-t-perspective-correct"></a>Screen-space barycentric weights aren't perspective-correct
Recall from
...
## <a id="the-heckbert-moreton-1z-trick"></a>The Heckbert-Moreton $1/z$ trick
...
## <a id="msaa-coverage-sampling-not-shading-sampling"></a>MSAA: coverage sampling, not shading sampling
...
## <a id="the-msaa-implementation"></a>The MSAA implementation
The
...
- **Single-tile path** (`m_msaaSamples == 1`): one float
  ...
  the [The rasterization pipeline](the-rasterization-pipeline.md) pipeline.
...
## <a id="the-cost-of-msaa"></a>The cost of MSAA
...
## <a id="where-the-perspective-correct-path-matters-most"></a>Where the perspective-correct path matters most
...
- **Depth.** Z-buffer correctness depends on perspective-correct
  ...
  [Clipping, depth, stencil: The depth test](clipping-depth-stencil.md#the-depth-test)
...
The rasterizer now has three anti-aliasing families implemented at different
points in the pipeline: MSAA during coverage, TAA across repeated frames, and
FXAA / SMAA as post-process filters. A few gaps remain:
...
SSAA remains a future addition. MSAA is still the canonical hardware-vintage
anti-aliasing technique, which makes it the right one to teach first.
...
- Previous: [Wireframe rendering](wireframe-rendering.md)
- Next volume:
  [Image processing & computer vision](../image-and-vision/README.md)
- Sampler partner:
...
- Edge-function barycentric weights:
  [The edge-function inside-test](the-rasterization-pipeline.md#the-edge-function-inside-test)
- Z-buffer:
  [The depth test](clipping-depth-stencil.md#the-depth-test)
