...
rasterizer ([Clipping, depth, stencil](../rasterization/clipping-depth-stencil.md))
— all `Matrix4d`. The 3×3 form (`Matrix3<double>`) shows up only
where translation is genuinely irrelevant, namely transforming
*directions* and *normals* ([Point vs direction vs normal](#point-vs-direction-vs-normal)).

## <a id="the-shape-of-matrix"></a>The shape of `Matrix<N, T>`
The class follows the same templated layout as the vector class
from [Numbers and vectors](numbers-and-vectors.md). From
...
Numerically sensitive work has a decomposition surface on
[`Matrix.h`](../../../include/core/math/Matrix.h). It provides LU with
partial pivoting for stable solves and inverses, QR for rebuilding an
orthonormal basis after drift, and SVD for breaking a small matrix into
orthogonal axes plus singular scales. The renderer's hot path still uses
`Matrix4::inverted()`, but `Matrix4::stableInverse()` can switch to LU
when the matrix is close enough to singular that the block inverse is the
wrong tradeoff.

## <a id="composition-order-column-vector-convention"></a>Composition order: column-vector convention
...
## <a id="point-vs-direction-vs-normal"></a>Point vs direction vs normal
...
[The four-matrix dance: `Instance`](#the-four-matrix-dance-instance).
...
perpendicular to the deformed surface. The [four-matrix dance](#the-four-matrix-dance-instance) widget shows
...
for you ([The four-matrix dance: `Instance`](#the-four-matrix-dance-instance)).

## <a id="quaternions-briefly"></a>Quaternions, briefly
...
## <a id="the-four-matrix-dance-instance"></a>The four-matrix dance: `Instance`
...
- **`m_normalMatrix`** is the *transpose* of `m_directionMatrix`.
  ...
  matrix the [normal-transform](#point-vs-direction-vs-normal) rule needs.
...
- Previous: [Numbers and vectors](numbers-and-vectors.md)
- Next: [Rays and geometry](rays-and-geometry.md)
