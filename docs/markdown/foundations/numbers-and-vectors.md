...
1. **Colors.** `Colord` (the [Color and buffers](color-and-buffers.md)
...
## <a id="the-shape-of-vector-n-t"></a>The shape of `Vector<N, T>`
...
## <a id="construction-indexing-and-the-obvious-operators"></a>Construction, indexing, and the obvious operators
...
- `operator*(const VectorType&)` is the dot product (see [Dot product](#dot-product)).
...
## <a id="dot-product"></a>Dot product
...
invariant in [Length, normalization, and the unit-length invariant](#length-normalization-and-the-unit-length-invariant) matters.
...
## <a id="cross-product"></a>Cross product
...
## <a id="length-normalization-and-the-unit-length-invariant"></a>Length, normalization, and the unit-length invariant
...
- Next chapter: [Matrices and transforms](matrices-and-transforms.md)
- Used in:
  - [Rays and geometry](rays-and-geometry.md) — origin,
    direction, the whole chapter rides on `Vector3d`
  - [Primitives and intersection](../ray-rendering/primitives-and-intersection.md) — hit-point math, per-shape derivations
  - [Materials and BRDFs](../ray-rendering/materials-and-brdfs.md) — the dot-product cosines from [Dot product](#dot-product)
