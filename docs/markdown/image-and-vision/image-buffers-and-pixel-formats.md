...
The ray-rendering, scene-structure, and rasterization chapters all spend their
time *filling* those buffers. Image processing and computer vision is about
*consuming* them: classical
...
- the pixel-type conventions this volume works with,
...
## <a id="the-pixel-formats-consumed-here"></a>The pixel formats consumed here
[Color and buffers: `Buffer<T>`: a fixed-size 2D array](../foundations/color-and-buffers.md#buffer-t-a-fixed-size-2d-array)
...
The image-processing helpers work exclusively with `Buffer<unsigned int>`. The
...
The pixel-type conversion API from [Color and buffers](../foundations/color-and-buffers.md)
is the bridge.
A `Colord` becomes its packed form via `.rgb()`; a packed pixel
gets read out as `.r()`, `.g()`, `.b()` integer accessors when
needed.

## <a id="the-standard-iteration-pattern"></a>The standard iteration pattern
...
## <a id="the-rendering-to-cv-handoff"></a>The rendering-to-CV handoff
...
This volume documents the synthetic-output algorithms the test suite actually
uses.

## <a id="the-cv-helpers-shipped-today"></a>The CV helpers shipped today
Three helpers under
...
[Blob analysis and silhouettes](blob-analysis-and-silhouettes.md) covers `Blob`
and `Silhouette` in depth; [Shape classification](shape-classification.md)
...
None of these ship today. This volume covers what the test suite consumes.
