...
publish finite BSDF lobes, while recursive reflective and transparent materials
still report compatibility shading until their delta lobes are represented
explicitly. This gives the graph, rendercli, and Modeler a separate executor
surface for the scheduler work that follows. After a traced wavefront render,
the pass metadata reports the selected
