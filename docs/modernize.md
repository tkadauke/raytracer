...
option(RAYTRACER_BUILD_EXAMPLES  "Build examples"       ON)
...
1. **Rendering progress events.** The tile-based parallel renderer should expose a callback or observer interface for progress reporting. The current `SceneBrowser` GUI presumably polls or blocks; an event-driven design makes headless rendering scriptable.
...
# Headless-only image — SceneBrowser excluded
...
- The GUI (`SceneBrowser`) is intentionally excluded from the runtime image — it requires a display server and is not suitable for headless cloud use.
...
- **CMake migration may break the macOS GUI workflow.** Qt's `AUTOMOC`/`AUTOUIC` in CMake handles `.moc` and `.uic` generation differently from the Rake `rule` blocks. Carefully test `SceneBrowser` compilation before removing the Rake fallback. Run both in parallel for at least one sprint.
