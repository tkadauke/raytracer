...
THEN(EngineFeatureTest, "the shadow boundary matches the tangent prediction") {
  ...
  ASSERT_FALSE(std::isnan(insidePixel.x()))
    << "shadowedInsideBoundary is behind the camera";
  ASSERT_FALSE(std::isnan(outsidePixel.x()))
    << "litOutsideBoundary is behind the camera";
  ...
  ASSERT_GE(cx, 0); ASSERT_LT(cx, 200);
  ASSERT_GE(cy, 0); ASSERT_LT(cy, 150);
  ASSERT_GE(ix, 0); ASSERT_LT(ix, 200);
  ASSERT_GE(iy, 0); ASSERT_LT(iy, 150);
  ASSERT_GE(ox, 0); ASSERT_LT(ox, 200);
  ASSERT_GE(oy, 0); ASSERT_LT(oy, 150);
  ...
  EXPECT_EQ(0u, test->colorAt(ix, iy))
    << "pixel (" << ix << "," << iy
    << ") just inside the tangent boundary should be black";
  ...
  EXPECT_GT(test->colorAt(ox, oy) >> 16, 0u)
    << "pixel (" << ox << "," << oy
    << ") just outside the tangent boundary should have red";
}
