...
WHEN(EngineFeatureTest, "i look at the origin") {
  test->lookAtOrigin();
}

WHEN(EngineFeatureTest, "i look away from the origin") {
  test->lookAway();
}

WHEN(EngineFeatureTest, "i go far away from the origin") {
  test->goFarAway();
}

WHEN(EngineFeatureTest, "i zoom by a factor of ([\\d.]+)") {
