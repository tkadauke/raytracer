...
namespace QuarticTest {
  ...
  TYPED_TEST(QuarticTest, ShouldSolveQuarticWithThreeResults) {
    ...
    ASSERT_CONTAINERS_NEAR(testing::makeStdVector<TypeParam>(-1, 0, 1), quartic.sortedResult(),
                           0.01);
  }

  TYPED_TEST(QuarticTest, ShouldSolveAnotherQuarticWithThreeResults) {
    Quartic<TypeParam> quartic(1, 1, -3, -1, 2);
    ASSERT_CONTAINERS_NEAR(testing::makeStdVector<TypeParam>(-2, -1, 1), quartic.sortedResult(),
                           0.01);
  }

  TYPED_TEST(QuarticTest, ShouldSolveQuarticWithFourResults) {
    Quartic<TypeParam> quartic(4, 0, -5, 0, 1);
    ASSERT_CONTAINERS_NEAR(testing::makeStdVector<TypeParam>(-1, -0.5, 0.5, 1),
                           quartic.sortedResult(), 0.01);
  }

  TYPED_TEST(QuarticTest, ShouldSolveAnotherQuarticWithFourResults) {
    Quartic<TypeParam> quartic(1, -16, 86, -176, 105);
    ASSERT_CONTAINERS_NEAR(testing::makeStdVector<TypeParam>(1, 3, 5, 7), quartic.sortedResult(),
                           0.01);
  }
  ...
  TYPED_TEST(QuarticTest, ShouldSolveQuarticWithWideCoefficientScale) {
    ...
    Quartic<TypeParam> quartic(TypeParam(1), TypeParam(-10000), TypeParam(-7), TypeParam(70006),
                               TypeParam(-60000));

    ASSERT_CONTAINERS_NEAR(testing::makeStdVector<TypeParam>(TypeParam(-3), TypeParam(1),
                                                             TypeParam(2), TypeParam(10000)),
                           quartic.stableSortedResult(), TypeParam(0.01));
  }
}
