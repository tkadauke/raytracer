...
namespace BinaryReadTest {
  ...
  TEST(BinaryRead, ReadsFloat32ArrayLittleEndianFromPointer) {
    ...
    const auto values = core::formats::readFloat32ArrayLe<3>(bytes.data());

    EXPECT_EQ((std::array<float, 3>{1.5f, -2.5f, 4.25f}), values);
  }

  TEST(BinaryRead, ReadsFloat32ArrayLittleEndianFromQByteArrayOffset) {
    ...
    const auto values = core::formats::readFloat32ArrayLe<3>(array, 4);
