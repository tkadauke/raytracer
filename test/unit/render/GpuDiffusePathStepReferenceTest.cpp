...
namespace GpuDiffusePathStepReferenceTest {
  ...
  TEST(GpuDiffusePrimarySampleChunks, CapturesFirstProgressDisplayThenSampleCompletions) {
    ...
    ASSERT_EQ(320u, chunks.size());
    EXPECT_LE(chunks.front().primaryPathGeneration.generatedPrimarySamples, 64u * 1024u);
    EXPECT_EQ(102, chunks.front().primaryPathGeneration.actualRect.height());
    ...
    ASSERT_LT(4u, chunks.size());
    EXPECT_TRUE(shouldCaptureGpuDiffusePathLoopChunkResolvedDisplay(settings, chunks[4]));
    EXPECT_EQ(408, chunks[4].primaryPathGeneration.actualRect.top());
    EXPECT_EQ(72, chunks[4].primaryPathGeneration.actualRect.height());
  ...
  TEST(GpuDiffusePrimarySampleChunks, ReportsTiledProgressOnFirstChunkAndSampleCompletion) {
    ...
    ASSERT_LT(15u, chunks.size());
    ASSERT_TRUE(chunks[15].completesSampleRange);
    ...
    notifyGpuDiffusePathLoopChunkProgress(settings, generation, chunks[15], platformChunk);
