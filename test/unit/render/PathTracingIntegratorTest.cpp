...
namespace PathTracingIntegratorTest {
  ...
  namespace {
    ...
    class SampleEchoLight final : public Light {
    public:
      Vector3d direction(const Vector3d&) const override {
        return Vector3d(0, 1, 0);
      }
      ...
      LightSample sample(const Vector3d&, const Vector2d& sample) const override {
        ...
        return {Vector3d(0, 1, 0), Colord(sample.x(), sample.y(), 0.0),
                std::numeric_limits<double>::infinity(), 1.0, true};
      }

      const char* fingerprintType() const override {
        return "SampleEchoLight";
      }
  ...
  TEST(PathTracingIntegrator, ScalarDirectLightingUsesSampleStreamLightDimension) {
    auto light = std::make_shared<SampleEchoLight>();
    auto scene = sampleEchoLightScene(light);

    PathTracingIntegrator integrator;
    integrator.setMaximumRecursionDepth(1);
    FixedLightSampleStream stream(Vector2d(0.25, 0.75));
    State state;
    ...
    ASSERT_COLOR_NEAR(Colord(0.25, 0.75, 0.0), pixel, 1e-12);
  ...
  TEST(PathTracingIntegrator, BatchedDirectLightingAveragesConfiguredDirectLightSamples) {
    ...
    EXPECT_EQ((std::vector<std::uint64_t>{metrics.directLightSelectionHostBytes}),
              metrics.directLightSelectionHostBytesPerDepth);
    EXPECT_EQ(2u, metrics.directLightOcclusionHostBytes);
    EXPECT_EQ((std::vector<std::uint64_t>{2u}), metrics.directLightOcclusionHostBytesPerDepth);
  ...
  TEST(PathTracingIntegrator, BatchedRadianceUsesPartialPacketFrontierForTwoActivePaths) {
    auto scene = std::make_unique<PacketCountingScene>();
    scene->setAmbient(Colord::black());
    scene->setBackground(Colord::black());
    ...
    EXPECT_EQ((std::vector<std::uint64_t>{0u}), metrics.frontierRayMissesPerDepth);
    EXPECT_EQ((std::vector<std::uint64_t>{1u}), metrics.frontierPacketChunksPerDepth);
    EXPECT_EQ((std::vector<std::uint64_t>{2u}), metrics.frontierPacketRaysPerDepth);
    EXPECT_EQ((std::vector<std::uint64_t>{1u}), metrics.frontierRay4PacketChunksPerDepth);
    EXPECT_EQ((std::vector<std::uint64_t>{0u}), metrics.frontierRay8PacketChunksPerDepth);
    EXPECT_EQ((std::vector<std::uint64_t>{0u}), metrics.frontierScalarRaysPerDepth);
    EXPECT_EQ((std::vector<std::uint64_t>{0u}), metrics.frontierPacketScalarFallbackRaysPerDepth);
  ...
  TEST(PathTracingIntegrator, BatchedRadianceUsesClosestHitBatchWhenBackendPrefersIt) {
    auto scene = simpleMatteScene(0.0, Colord(0.6, 0.3, 0.2));
    PathTracingIntegrator integrator;
    integrator.setMaximumRecursionDepth(1);
    ...
    BatchPreferringCountingIntersectionBackend backend;
    IntegratorBatchSettings settings;
    settings.intersectionBackend = &backend;
    FallbackRayCaster caster;
    IntegratorBatchMetrics metrics;
  ...
  TEST(PathTracingIntegrator, BatchedDirectLightingUsesConfiguredBackendForBoundedVisibility) {
    ...
    EXPECT_EQ((std::vector<std::uint64_t>{metrics.directLightSelectionHostBytes}),
              metrics.directLightSelectionHostBytesPerDepth);
    EXPECT_EQ(1u, metrics.directLightOcclusionHostBytes);
    EXPECT_EQ((std::vector<std::uint64_t>{1u}), metrics.directLightOcclusionHostBytesPerDepth);
  ...
  TEST(PathTracingIntegrator, BatchedDirectLightingRejectsMissingVisibilityFrontier) {
    auto scene = std::make_unique<Scene>(Colord::black());
    scene->setAmbient(Colord::black());
    scene->setBackground(Colord::black());
    ...
    EXPECT_THROW(integrator.radianceBatch(*scene, samples, caster, &metrics, settings),
                 std::logic_error);
    EXPECT_EQ((std::vector<std::size_t>{2u}), backend.requestedQueryCounts);
  }

  TEST(PathTracingIntegrator, BatchedDirectLightingRejectsMismatchedVisibilityFrontier) {
    auto scene = std::make_unique<Scene>(Colord::black());
    scene->setAmbient(Colord::black());
    scene->setBackground(Colord::black());
  ...
  TEST(PathTracingIntegrator, BatchedRadianceUsesRay8PacketFrontierForEightActivePaths) {
    ...
    countingScene->setAmbient(scene->ambient());
    countingScene->setBackground(scene->background());
    for (const auto& primitive : scene->primitives()) {
      countingScene->add(primitive);
    }
    ...
    IntegratorBatchMetrics metrics;
    const std::vector<Colord> batched =
      integrator.radianceBatch(*countingScene, samples, caster, &metrics);

    ASSERT_EQ(8u, batched.size());
    EXPECT_EQ(0, countingScene->packet4HitCalls);
    EXPECT_EQ(1, countingScene->packet8HitCalls);
    EXPECT_EQ((std::vector<std::uint64_t>{8u}), metrics.frontierRayHitsPerDepth);
    EXPECT_EQ((std::vector<std::uint64_t>{0u}), metrics.frontierRayMissesPerDepth);
    EXPECT_EQ((std::vector<std::uint64_t>{1u}), metrics.frontierPacketChunksPerDepth);
  ...
  TEST(PathTracingIntegrator, BatchedRadianceQueuesOnlyStillActivePathsAtNextDepth) {
    ...
    EXPECT_EQ(metrics.activeHostPathStateBytesProcessed,
              metrics.frontierCompactionInputHostPathStateBytes);
    EXPECT_EQ(0u, metrics.frontierCompactionRetainedHostPathStateBytes);
    EXPECT_EQ(metrics.activeHostPathStateBytesProcessed,
  ...
  TEST(PathTracingIntegrator, EstimatesIntersectionRaysFromBouncesAndDirectLightSamples) {
    ...
    EXPECT_EQ(8u, integrator.estimatedClosestHitRaysPerPrimarySample());
    EXPECT_EQ(8u, integrator.estimatedAnyHitRaysPerPrimarySample());

    integrator.setMaximumRecursionDepth(3);
    integrator.setDirectLightSamples(4);

    EXPECT_EQ(15u, integrator.estimatedIntersectionRaysPerPrimarySample());
    EXPECT_EQ(3u, integrator.estimatedClosestHitRaysPerPrimarySample());
    EXPECT_EQ(12u, integrator.estimatedAnyHitRaysPerPrimarySample());
    ...
    EXPECT_EQ(1u, integrator.estimatedAnyHitRaysPerPrimarySample());
  }
