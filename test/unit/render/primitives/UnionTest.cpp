...
using namespace render;
  ...
  TEST(Union, ShouldNotReturnAnyPrimitiveIfThereIsNoIntersection) {
    ...
    u.add(primitive1);
    u.add(primitive2);
    EXPECT_CALL(*primitive1.get(), intersect(_, _, _)).WillOnce(Return(static_cast<render::Primitive*>(nullptr)));
    EXPECT_CALL(*primitive2.get(), intersect(_, _, _)).WillOnce(Return(static_cast<render::Primitive*>(nullptr)));
