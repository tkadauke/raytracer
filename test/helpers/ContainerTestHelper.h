  ...
  namespace internal {
    ...
    bool containersEqual(const Container& expected, const Container& actual) {
      if (expected.size() != actual.size())
        return false;
      for (typename Container::const_iterator i = expected.begin(), j = actual.begin();
           i != expected.end(); ++i, ++j) {
        if (!(*i == *j))
          return false;
      }
    ...
    AssertionResult ContainersEqPredFormat(const char* expr1, const char* expr2,
                                           const Container& val1, const Container& val2) {
      if (containersEqual(val1, val2))
        return AssertionSuccess();

      Message msg;
      msg << "The containers " << expr1 << " and " << expr2 << " are not equal, where\n"
          << expr1 << " evaluates to ";
