...
namespace testing {
  namespace internal {
    ...
    bool matrixNear(const Matrix<Dimensions, T, VectorType, Derived>& expected,
      ...
      for (int row = 0; row != Dimensions; ++row) {
        for (int col = 0; col != Dimensions; ++col) {
          if (actual[row][col] < expected[row][col] - threshold ||
              actual[row][col] > expected[row][col] + threshold)
            return false;
        }
      }
      return true;
    ...
    MatrixNearPredFormat(const char* expr1, const char* expr2, const char* abs_error_expr,
      ...
      if (matrixNear(val1, val2, T(abs_error)))
        return AssertionSuccess();

      Message msg;
      msg << "The difference between matrixes " << expr1 << " and " << expr2 << " exceeds "
