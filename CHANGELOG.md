...
- Let sampled-shutter equirectangular, spherical, and fish-eye camera rig
  translations over one linear shutter segment lower to GPU primary-path
  descriptors by carrying a primary-origin motion delta through CPU reference,
  Metal, and Vulkan path-loop launches. — GPT-5 Codex
- Let sampled-shutter orthographic camera rigs whose linear in-shutter motion
  changes their view direction stay on GPU primary-path descriptors by carrying
...
- Let fixed-shutter animated orthographic, thin-lens, tilt-shift,
  ...
  animated camera poses. — GPT-5 Codex
...
- Report compiled GPU path-loop capture policy in render graph trace metadata,
...
- Let simple trace-disabled graph LDR renders fed by a full-GPU path tracer use
  the platform-resolved display buffer as the final linear-tonemap output,
  avoiding HDR accumulation readback when no graph pass needs that HDR resource.
  — GPT-5 Codex
- Stop requesting platform display-resolve readback from trace-disabled
...
- Allow restricted Vulkan full-GPU diffuse path-loop renders to accumulate
  multiple samples for the same output pixel through platform sample-slot
  accumulation instead of rejecting duplicate active pixel targets. —
  GPT-5 Codex
...
- Extend the restricted Vulkan full-GPU diffuse path-loop backend from
  one-sphere geometry to one untransformed sphere or plane for its supported
  material, texture, and light subset. — GPT-5 Codex
- Extend the restricted Vulkan full-GPU diffuse path-loop backend to accept
  multiple untransformed sphere, plane, rectangle, and disk primitives for its
  supported material, texture, and light subset. — GPT-5 Codex
- Extend the restricted Vulkan full-GPU diffuse path-loop backend to accept
...
- Add a restricted Metal diffuse path-loop backend surface for one-depth
  ...
  exists. — GPT-5 Codex
- Extend the restricted Metal diffuse path-loop backend with a single-dispatch
  multi-depth Matte/Emissive sphere path loop, while keeping automatic full-GPU
...
- Add GPU-generated retained frontier indices to the Metal diffuse path-loop
  probes, using a count-prefixed retained-index buffer in the launch ABI. —
  GPT-5 Codex
...
- Use a smaller automatic full-GPU path-tracer launch budget when the render
  ...
  GPT-5 Codex
- Keep interactive full-GPU path-tracer auto chunking under the same hard
  per-launch cap as batch renders, so high-depth preview renders stay
  cancellable instead of blocking on full-frame sample dispatches. —
  GPT-5 Codex
- Show automatic GPU path-tracer sample chunking as `Auto` in the render dialog
  instead of displaying the raw sentinel value `0`. — GPT-5 Codex
...
- Let denoiser feature capture share the same chunked full-GPU path-tracer
  launch path by merging per-pixel feature records across sample chunks instead
  of forcing one large launch. — GPT-5 Codex
...
- Make the Modeler final render dialog's GPU sample stream an explicit Path
...
- Reject standalone intersection-service direct-light visibility results whose
  ...
  — GPT-5 Codex
- Reject default wavefront backend direct-light visibility batches whose
  frontier any-hit execution returns an occlusion count that does not match the
  frontier ray count. — GPT-5 Codex
...
- Deduplicate `tangentFor`, `cosineHemisphereDirection`, and `phongLobeDirection`
  sampling helpers that were copied identically into `Lambertian.cpp`,
...
- Reuse the Vulkan full-GPU diffuse path-loop instance, device, queue, descriptor
  ...
  avoiding per-image Vulkan runtime setup and buffer allocation on steady
  platform path-tracing launches. — GPT-5 Codex
...
- Route `IntersectionService` direct-light visibility through the same
  backend-owned batch hook used by ray-family integrators, keeping standalone
  GPU direct-light CPU-reference consumers on the resident-direct-light
  diagnostics path. — GPT-5 Codex
...
- Print structured tracing restricted capability summaries in rendercli
  ...
  report. — GPT-5 Codex
- Show intersection-service frontier residency and packed/host/state payload
  byte diagnostics in Modeler render graph selected-pass details. — GPT-5 Codex
...
- Track intersection-service closest-hit and any-hit query counts, hit counts,
  occlusion counts, and transfer-byte estimates directly on the reusable
  service diagnostics so graph consumers share one backend-owned summary.
  — GPT-5 Codex
- Print compact intersection-service diagnostics for graph passes when rendercli
  metrics summaries are enabled, covering hybrid visibility and ray-traced
  shadow passes. — GPT-5 Codex
...
- Report frontier compaction path-state residency in wavefront metrics,
  rendercli summaries, and Modeler render graph details. — GPT-5 Codex
...
- Exposed intersection-service query family, backend path, counts, and transfer
  byte estimates in Modeler render graph selected-pass details. — GPT-5 Codex
...
- Routed path-tracing closest-hit batch work through backend-owned frontier
  ...
  vectors. — GPT-5 Codex
- Routed Whitted closest-hit batch work through backend-owned frontier handles
  so Whitted and path tracing report the same frontier residency diagnostics.
  — GPT-5 Codex
...
- Reported state-handle byte estimates for wavefront compaction candidates in
  ...
  details. — GPT-5 Codex
- Reported retained-index byte estimates for executed wavefront frontier
  compaction in metrics, rendercli summaries, convergence captures, benchmarks,
...
- Reported and preserved why resident direct-light batches are unavailable
...
- Exposed last active and retained wavefront host path-state byte counts in
  Modeler graph-node tooltips and selected-pass graph details. — GPT-5 Codex
- Reported path-tracing spawned-continuation counts and host path-state bytes in
  ...
  — GPT-5 Codex
- Reported Whitted wavefront queued-ray state bytes through the same
  active/retained host path-state diagnostics used by path tracing.
  — GPT-5 Codex
...
- **Vulkan triangle any-hit dispatch.** Vulkan-enabled builds now expose a
  ...
  coverage. — GPT-5
- **Vulkan triangle wavefront backend execution.** Vulkan-enabled builds can now
  route prepared untransformed triangle scenes through Vulkan closest-hit and
  any-hit wavefront intersection batches; non-triangle/basic scenes continue to
  fall back to packed CPU traversal until matching Vulkan kernels exist.
  — GPT-5
...
- **Wavefront resident-frontier capability diagnostics.** Wavefront metrics,
...
- **Wavefront largest-compaction fraction.** Wavefront metrics, rendercli
  summaries, and the Modeler graph details now report the inactive-sample
  fraction at the largest compaction candidate depth. — GPT-5 Codex
...
- **Wavefront unsupported-reason metrics.** Wavefront backend diagnostics now
  ...
  pass details, so unsupported GPU requests are inspectable without parsing the
  fallback sentence. — GPT-5
...
- **Wavefront expected query-family diagnostics.** Wavefront metrics JSON,
  ...
  transfer estimate uses matching hit/occlusion readback sizes. — GPT-5
...
- **Wavefront GPU CMake presets.** Added platform-conditional
  ...
  Metal/Vulkan wavefront intersection builds can be configured without custom
  CMake flags. — GPT-5
- **Wavefront auto backend threshold diagnostics.** Wavefront metrics JSON,
  rendercli summaries, and Modeler graph pass metadata now report the
  auto-selection minimum GPU ray threshold beside the expected ray count, so
  small renders explain why `auto` stayed on CPU. — GPT-5
...
- **Wavefront intersection ray-family diagnostics.** Wavefront metrics JSON,
  ...
  direct-light visibility visible as both query count and submitted shadow-ray
  count. — GPT-5
...
- **Path-tracing contribution diagnostics.** Wavefront/pathtracer metrics,
  render graph trace metadata, Modeler graph tooltips, and rendercli compact
...
- **Path tracer direct-light sample controls.** Render intent, graph JSON,
...
- **Render-to-texture screen documentation and validation coverage.** The
  render graph plan and textbook now document screen receivers, nested subview
  branches, Modeler/export behavior, concrete rendercli verification commands,
  and cyclic render-to-texture graph rejection. — GPT-5
...
- **Render-to-texture material binding.** Graph execution now binds CPU-backed
  ...
  tracing its CPU fallback instead of silently ignoring the generated subview.
  — GPT-5
...
- **Wavefront capture packet/tile comparisons.** The convergence capture script
...
- **Wavefront partial Ray8 tails.** Whitted and path-tracing wavefront
  ...
  work. — GPT-5
- **Wavefront metric snapshot gating.** Wavefront packet frontiers now allocate
  packet-fallback reason snapshots only when metrics are active, avoiding
  per-packet diagnostic map construction in ordinary Whitted and path-tracing
  wavefront renders. — GPT-5
...
- **Wavefront path survivor compaction.** Path-tracing batches now compact
  surviving paths in place between depths, avoiding a second survivor vector in
  the hot wavefront batch loop. — GPT-5
...
- **Packed wavefront primitive-material fallback.** Prepared GPU-style
  ...
  the compiled CPU closest-hit path. — GPT-5
...
- **Packed wavefront payload normals.** Packed CPU, Metal, and Vulkan
  ...
  transform handling, matching the compiled/runtime hit metadata contract. —
  GPT-5
