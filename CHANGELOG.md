...
- **World and rendercli LDraw imports.** Scene JSON can now contain
  ...
  geometry, or explicit `LDrawModel` surfaces with file path, library root,
  transform scale, smoothing, and visibility properties; direct LDraw input
  remains available with `--ldraw_library_root` for #210. — GPT-5
