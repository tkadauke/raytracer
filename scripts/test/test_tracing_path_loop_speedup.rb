...
class TracingPathLoopSpeedupVerifierTest < Minitest::Test
  ...
  def gpu_row(paths:, seconds:, supported: 1.0, unavailable: 0.0)
    benchmark_row("bm_requestedGpuCompiledDiffusePathLoopWarmedSceneUpload/3/#{paths}/4",
                  seconds,
  ...
  def test_rejects_gpu_row_that_was_skipped_by_benchmark
    result = verify([cpu_row(paths: 65_536, seconds: 2.0),
                     {
                       "name" => "bm_requestedGpuCompiledDiffusePathLoopWarmedSceneUpload/3/65536/4",
                       "error_occurred" => true,
