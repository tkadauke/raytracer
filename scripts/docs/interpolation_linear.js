((scriptElement) => {
  class InterpolationLinearWidget {
    constructor() {
      this.t = 0.35;
      this.width = 560;
      this.height = 290;
    ...
    element() {
      ...
      this.widget = new FigureWidget({ className: 'interpolation-linear-widget' });
    ...
    renderAxes() {
      ...
      this.canvas.arrow(new Vector(left, bottom), new Vector(right + 26, bottom), {
        ...
        markerId: 'interpolation-linear-t-axis',
      });
      this.canvas.arrow(new Vector(left, bottom), new Vector(left, top - 20), {
        stroke: '#222',
        'stroke-width': FigurePixelGuideStrokeWidth,
        markerId: 'interpolation-linear-value-axis',
      });
    ...
    render() {
      this.canvas.clear();
      this.canvas.element.setAttribute('data-interpolator', 'linear');
      ...
      this.canvas.line(new Vector(x0, y0), new Vector(x1, y1), {
        stroke: '#2f9e44',
      ...
      this.canvas.text(64, 260, `sample = ${value.toFixed(2)}`, { 'font-size': 13, fill: '#333' });
    }
  }

  const figure = new InterpolationLinearWidget();
