...
var ThinLensConvergence = new Class({
  initialize: function() {
  ...
  createCanvas: function() {
    ...
    // Eye position
    var eye = new Vector(-this.distance, 0);
    ...
    var pixel = new Vector(0, this.pixelY);
    ...
    var pinholeDir = pixel.minus(eye);
    ...
    var tFocal = (this.focalDistance - eye.x) / pinholeDir.x;
    var focalPoint = eye.plus(pinholeDir.multiply(tFocal));
    ...
    for (var i = 0; i < this.numRays; i++) {
      var t = this.numRays === 1 ? 0.5 : i / (this.numRays - 1);
      var lensY = -this.apertureRadius + t * 2 * this.apertureRadius;
      var lensPoint = new Vector(-this.distance, lensY);

      // Direction from lens point through focal point
      var rayDir = focalPoint.minus(lensPoint);
      // Extend the ray well past the focal plane (1.6× distance to focal)
      var ray = rayDir.multiply(1.6);
