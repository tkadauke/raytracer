class BoundingBoxOr {
  ...
  element() {
    ...
    this.widget = new FigureWidget({ className: 'bounding-box-or-widget' });
  ...
  union() {
    ...
    const left = Math.min(a.topleft.x, b.topleft.x);
    const top = Math.min(a.topleft.y, b.topleft.y);
    const right = Math.max(a.topleft.x + a.size.x, b.topleft.x + b.size.x);
    const bottom = Math.max(a.topleft.y + a.size.y, b.topleft.y + b.size.y);
    return {
