...
Surface* Grid::intersect(const Ray& ray, HitPointInterval& hitPoints) {
  ...
  while (true) {
    Surface* surface = m_cells[x + m_numX * y + m_numX * m_numY * z];

    if (tx_next < ty_next && tx_next < tz_next) {
      if (surface) {
        HitPointInterval candidate;
        surface = surface->intersect(ray, candidate);
...
bool Grid::intersects(const Ray& ray) {
  ...
  while (true) {
    ...
    if (tx_next < ty_next && tx_next < tz_next) {
      if (surface && surface->intersects(ray))
        return true;
...
void Grid::setup() {
  ...
  float s = pow(gridSize.x() * gridSize.y() * gridSize.z() / surfaces().size(), 0.3333333);
  ...
  for (Surfaces::const_iterator surface = surfaces().begin(); surface != surfaces().end(); ++surface) {
    BoundingBox bbox = (*surface)->boundingBox();
      ...
      for (int y = ymin; y <= ymax; ++y) {
        for (int x = xmin; x <= xmax; ++x) {
          ...
          if (counts[index] == 0) {
            ...
            dynamic_cast<Composite*>(m_cells[index])->add(*surface);
          }
          counts[index]++;
        }
      }
