...
RayPacketIntersection4 Triangle::intersectPacket(const Ray4& rays, render::State& state) const {
  ...
  const __m128 d = _mm_sub_ps(_mm_set1_ps(static_cast<float>(m_point0.x())), _mm_load_ps(rays.originX.data()));
  ...
  const __m128 h = _mm_sub_ps(_mm_set1_ps(static_cast<float>(m_point0.y())), _mm_load_ps(rays.originY.data()));
  ...
  const __m128 l = _mm_sub_ps(_mm_set1_ps(static_cast<float>(m_point0.z())), _mm_load_ps(rays.originZ.data()));
  ...
  const __m128 denom = _mm_add_ps(_mm_add_ps(_mm_mul_ps(a, mm), _mm_mul_ps(b, q)), _mm_mul_ps(c, s));
