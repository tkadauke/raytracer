  ...
  namespace {
    ...
    NSString* diffusePathLoopKernelSourcePrefix() {
      return @"#include <metal_stdlib>\n"
              ...
              "    bool childSupported = false;\n"
              "    const float4 child = untintedTextureColor(\n"
              "        parameters, sceneUpload, texture.payloadOffset, hit, childSupported);\n"
              "    if (!childSupported) {\n"
              "      return float4(0.0f);\n"
              "    }\n"
              "    supported = true;\n"
              "    return child * texture.parameters;\n"
              "  }\n"
              "  return untintedTextureColor(parameters, sceneUpload, textureId, hit, supported);\n"
              "}\n";
    }
