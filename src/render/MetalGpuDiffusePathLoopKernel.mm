  ...
  namespace {
    ...
    NSString* diffusePathLoopKernelSourcePrefix() {
      return @"#include <metal_stdlib>\n"
              ...
              "    if (texture.payloadOffset >= parameters.textureCount) {\n"
              "      return float4(0.0f);\n"
              "    }\n"
              "    const GpuTracingTextureRecord child = textures[texture.payloadOffset];\n"
              "    if (child.kind != gpuTracingConstantColorTextureKind) {\n"
              "      return float4(0.0f);\n"
              "    }\n"
              "    supported = true;\n"
              "    return child.parameters * texture.parameters;\n"
              "  }\n"
