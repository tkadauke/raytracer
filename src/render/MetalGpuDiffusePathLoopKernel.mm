...
namespace render {
  ...
  namespace {
    ...
    static_assert(sizeof(GpuTracingMaterialRecord) == 192);
    ...
    NSString* diffusePathLoopKernelSourcePrefix() {
      return @"#include <metal_stdlib>\n"
              ...
              "  float4 primaryPathLensRight;\n"
              "  float4 primaryPathLensUp;\n"
              "  float4 primaryPathForward;\n"
              "  float4 primaryPathLensParameters;\n"
              "};\n"
              ...
              "  float4 portalOriginMatrix0;\n"
              "  float4 portalOriginMatrix1;\n"
              "  float4 portalOriginMatrix2;\n"
              "  float4 portalOriginMatrix3;\n"
              "  float4 portalDirectionMatrix0;\n"
              "  float4 portalDirectionMatrix1;\n"
              "  float4 portalDirectionMatrix2;\n"
              "};\n"
              ...
              "constant uint gpuTracingPortalMaterialKind = 6u;\n"
              ...
              "constant uint gpuPrimaryPathGenerationModeOrthographic = 2u;\n"
              "constant uint gpuPrimaryPathGenerationModeThinLens = 3u;\n"
              "constant uint gpuPrimaryPathGenerationModeEquirectangular = 4u;\n"
              "constant uint gpuPrimaryPathGenerationModeSpherical = 5u;\n"
              "constant uint gpuPrimaryPathGenerationModeFishEye = 6u;\n"
              "constant uint gpuPrimaryPathGenerationModeTiltShift = 7u;\n"
              ...
              "                                  dimension, 1u));\n"
              "}\n"
              "float2 concentricMapToDisc(float2 sample) {\n"
              ...
              "  if (a * a > b * b) {\n"
              "    r = a;\n"
              "    phi = (pathLoopTau / 8.0f) * (b / a);\n"
              "  } else {\n"
              "    r = b;\n"
              "    phi = (pathLoopTau / 4.0f) - (pathLoopTau / 8.0f) * (a / b);\n"
              "  }\n"
              "  return float2(r * cos(phi), r * sin(phi));\n"
              "}\n"
              "float3 orthographicLookAtWorldPoint(float3 position, float3 forward,\n"
              "                                     float3 localPoint) {\n"
              "  const float3 right = normalize(cross(float3(0.0f, 1.0f, 0.0f), forward));\n"
              "  const float3 down = cross(right, -forward);\n"
              "  return position + right * localPoint.x + down * localPoint.y +\n"
              ...
              "  float3 forward;\n"
              "};\n"
              "PrimaryPathPanoramicBasis primaryPathPanoramicBasis(\n"
              "    constant GpuDiffusePathLoopLaunchParameters& parameters,\n"
              "    float timeSample, float originOffset) {\n"
              "  PrimaryPathPanoramicBasis basis;\n"
              "  basis.origin = (parameters.primaryPathOrigin +\n"
              "                  parameters.primaryPathMotionOriginDelta * timeSample).xyz;\n"
              ...
              "    const float3 position = basis.origin;\n"
              ...
              "    basis.right = normalize(cross(float3(0.0f, 1.0f, 0.0f), basis.forward));\n"
              "    basis.down = cross(basis.right, -basis.forward);\n"
              "    basis.origin = position - basis.forward * originOffset;\n"
              "  }\n"
              "  return basis;\n"
              "}\n"
              "float3 primaryPathRectilinearWorldPoint(PrimaryPathPanoramicBasis basis,\n"
              "                                        float cameraDistance,\n"
              "                                        float3 localPoint) {\n"
              "  return basis.origin + basis.forward * cameraDistance +\n"
              "      basis.right * localPoint.x + basis.down * localPoint.y +\n"
              ...
              "      parameters.primaryPathRight * (float(column) + pixelSample.x) +\n"
              "      parameters.primaryPathDown * (float(row) + pixelSample.y);\n"
              "  float4 rayOrigin = pixelPoint +\n"
              "      parameters.primaryPathMotionOriginDelta * timeSample;\n"
              "  float3 rayDirection = normalize(parameters.primaryPathOrigin.xyz);\n"
              "  if (parameters.primaryPathMotionMode == gpuPrimaryPathMotionModeLookAt) {\n"
              "    const float3 position = parameters.primaryPathOrigin.xyz +\n"
              "        parameters.primaryPathMotionOriginDelta.xyz * timeSample;\n"
              "    const float3 target = parameters.primaryPathMotionTarget.xyz +\n"
              "        parameters.primaryPathMotionTargetDelta.xyz * timeSample;\n"
              "    rayDirection = normalize(target - position);\n"
              "    rayOrigin = float4(\n"
              "        orthographicLookAtWorldPoint(position, rayDirection, pixelPoint.xyz), 1.0f);\n"
              "  }\n"
              "  GpuDiffusePathStateRecord path;\n"
              "  path.ray.origin = rayOrigin;\n"
              "  path.ray.direction = float4(rayDirection, 0.0f);\n"
              "  path.ray.minDistance = 0.0f;\n"
              "  path.ray.maxDistance = rayInfinity();\n"
              "  path.ray.timeSample = timeSample;\n"
              "  path.ray.flags = 0u;\n"
              ...
              "  const float timeSample = sample1DCoordinate(\n"
              "      parameters.primaryPathSampleSeed, pixelIndex, sampleIndex, 1u, 0u);\n"
              "  const float2 lensSample = concentricMapToDisc(sample2DCoordinate(\n"
              "      parameters.primaryPathSampleSeed, pixelIndex, sampleIndex, 2u));\n"
              "  const float4 pixelPoint =\n"
              "      parameters.primaryPathTopLeft +\n"
              "      parameters.primaryPathRight * (float(column) + pixelSample.x) +\n"
              "      parameters.primaryPathDown * (float(row) + pixelSample.y);\n"
              "  const bool lookAtMotion =\n"
              "      parameters.primaryPathMotionMode == gpuPrimaryPathMotionModeLookAt;\n"
              "  const PrimaryPathPanoramicBasis basis = primaryPathPanoramicBasis(\n"
              "      parameters, timeSample,\n"
              "      lookAtMotion ? parameters.primaryPathMotionParameters.x : 0.0f);\n"
              "  const float3 focalForward = normalize(\n"
              "      lookAtMotion ? basis.forward : parameters.primaryPathForward.xyz);\n"
              "  const float3 worldPixelPoint = lookAtMotion ?\n"
              "      primaryPathRectilinearWorldPoint(\n"
              "          basis, parameters.primaryPathMotionParameters.x, pixelPoint.xyz) :\n"
              "      pixelPoint.xyz;\n"
              "  const float4 origin = float4(basis.origin, 1.0f);\n"
              "  const float3 pinholeDirection =\n"
              "      normalize(worldPixelPoint - origin.xyz);\n"
              "  const float t = parameters.primaryPathLensParameters.x /\n"
              "                  dot(pinholeDirection, focalForward);\n"
              "  const float3 focalPoint = origin.xyz + pinholeDirection * t;\n"
              "  const float4 lensRight = lookAtMotion ?\n"
              "      float4(basis.right * length(parameters.primaryPathLensRight.xyz), 0.0f) :\n"
              "      parameters.primaryPathLensRight;\n"
              "  const float4 lensUp = lookAtMotion ?\n"
              "      float4(basis.down * length(parameters.primaryPathLensUp.xyz), 0.0f) :\n"
              "      parameters.primaryPathLensUp;\n"
              "  const float4 lensOrigin = origin + lensRight * lensSample.x +\n"
              "                            lensUp * lensSample.y;\n"
              "  GpuDiffusePathStateRecord path;\n"
              "  path.ray.origin = lensOrigin;\n"
              "  path.ray.direction = float4(normalize(focalPoint - lensOrigin.xyz), 0.0f);\n"
              "  path.ray.minDistance = 0.0f;\n"
              "  path.ray.maxDistance = rayInfinity();\n"
              "  path.ray.timeSample = timeSample;\n"
              "  path.ray.flags = 0u;\n"
              ...
              "  const float lon = (2.0f * x / parameters.primaryPathLensParameters.x - 1.0f) *\n"
              "                    (pathLoopTau * 0.5f);\n"
              "  const float lat = (1.0f - 2.0f * y / parameters.primaryPathLensParameters.y) *\n"
              "                    (pathLoopTau * 0.25f);\n"
              "  const float cosLat = cos(lat);\n"
              "  const float3 local = float3(cosLat * sin(lon), -sin(lat), cosLat * cos(lon));\n"
              "  const PrimaryPathPanoramicBasis basis =\n"
              "      primaryPathPanoramicBasis(parameters, timeSample, 0.0f);\n"
              "  const float3 direction = normalize(basis.right * local.x +\n"
              "                                     basis.down * local.y +\n"
              "                                     basis.forward * local.z);\n"
              "  GpuDiffusePathStateRecord path;\n"
              "  path.ray.origin = float4(basis.origin, 1.0f);\n"
              "  path.ray.direction = float4(direction, 0.0f);\n"
              "  path.ray.minDistance = 0.0f;\n"
              "  path.ray.maxDistance = rayInfinity();\n"
              "  path.ray.timeSample = timeSample;\n"
              "  path.ray.flags = 0u;\n"
              ...
              "  const float x = float(column) + pixelSample.x;\n"
              "  const float y = float(row) + pixelSample.y;\n"
              "  const float pointX = 2.0f / parameters.primaryPathLensParameters.x * x + 1.0f;\n"
              "  const float pointY = 2.0f / parameters.primaryPathLensParameters.y * y - 1.0f;\n"
              "  const float lambda = pointX * 0.5f * parameters.primaryPathLensParameters.z;\n"
              "  const float psi = pointY * 0.5f * parameters.primaryPathLensParameters.w;\n"
              "  const float phi = (pathLoopTau * 0.5f) - lambda;\n"
              "  const float theta = (pathLoopTau * 0.25f) - psi;\n"
              "  const float sinPhi = sin(phi);\n"
              "  const float cosPhi = cos(phi);\n"
              "  const float sinTheta = sin(theta);\n"
              "  const float cosTheta = cos(theta);\n"
              "  const float3 local = float3(sinTheta * sinPhi, cosTheta,\n"
              "                              sinTheta * cosPhi);\n"
              "  const PrimaryPathPanoramicBasis basis = primaryPathPanoramicBasis(\n"
              "      parameters, timeSample, parameters.primaryPathMotionParameters.x);\n"
              "  const float3 direction = normalize(basis.right * local.x +\n"
              "                                     basis.down * local.y +\n"
              "                                     basis.forward * local.z);\n"
              "  GpuDiffusePathStateRecord path;\n"
    ...
    NSString* diffusePathLoopKernelSourcePrimaryPathDescriptors() {
      return @"GpuDiffusePathStateRecord makeFishEyePrimaryPath(\n"
              ...
              "      2.0f / parameters.primaryPathLensParameters.x * x - 1.0f,\n"
              "      2.0f / parameters.primaryPathLensParameters.y * y - 1.0f);\n"
              "  const float r2 = dot(point, point);\n"
              "  const bool valid = r2 <= 1.0f && r2 > 0.0f;\n"
              "  const PrimaryPathPanoramicBasis basis =\n"
              "      primaryPathPanoramicBasis(parameters, timeSample, 0.0f);\n"
              "  float3 direction = normalize(basis.forward);\n"
              "  if (valid) {\n"
              "    const float r = sqrt(r2);\n"
              "    const float psi = r * parameters.primaryPathLensParameters.z * 0.5f;\n"
              "    const float sinPsi = sin(psi);\n"
              "    const float cosPsi = cos(psi);\n"
              "    const float sinAlpha = point.y / r;\n"
              "    const float cosAlpha = point.x / r;\n"
              "    const float3 local = float3(sinPsi * cosAlpha, sinPsi * sinAlpha, cosPsi);\n"
              "    direction = normalize(basis.right * local.x + basis.down * local.y +\n"
              "                          basis.forward * local.z);\n"
              "  }\n"
              "  GpuDiffusePathStateRecord path;\n"
              "  path.ray.origin = float4(basis.origin, 1.0f);\n"
              ...
              "  path.sampleDimensionBase = gpuPrimaryPathSampleDimensionBase;\n"
              "  path.sampleDimensionStride = gpuPrimaryPathSampleDimensionStride;\n"
              "  path.flags = valid ? gpuDiffusePathStateActiveFlag :\n"
              "                       gpuDiffusePathStateTerminatedFlag;\n"
              "  path.reserved0 = 0u;\n"
              "  path.previousBsdfPdf = 0.0f;\n"
              "  path.previousLightPdf = 0.0f;\n"
              ...
              "      parameters, timeSample,\n"
              "      lookAtMotion ? parameters.primaryPathMotionParameters.x : 0.0f);\n"
              "  const float3 rightBasis = normalize(\n"
              "      lookAtMotion ? basis.right : parameters.primaryPathRight.xyz);\n"
              "  const float3 upBasis = normalize(\n"
              "      lookAtMotion ? basis.down : parameters.primaryPathDown.xyz);\n"
              "  const float3 focalForward = normalize(\n"
              "      lookAtMotion ? basis.forward : parameters.primaryPathForward.xyz);\n"
              "  const float3 worldPixelPoint = lookAtMotion ?\n"
              "      primaryPathRectilinearWorldPoint(\n"
              "          basis, parameters.primaryPathMotionParameters.x, pixelPoint.xyz) :\n"
              "      pixelPoint.xyz;\n"
              "  const float3 shiftedPixelPoint =\n"
              "      worldPixelPoint + rightBasis * parameters.primaryPathLensParameters.y +\n"
              "      upBasis * parameters.primaryPathLensParameters.z;\n"
              "  const float4 origin = float4(basis.origin, 1.0f);\n"
              "  const float3 pinholeDirection =\n"
              "      normalize(shiftedPixelPoint - origin.xyz);\n"
              "  const float tilt = parameters.primaryPathLensParameters.w;\n"
              "  const float3 tiltedNormal = focalForward * cos(tilt) +\n"
              "                               cross(rightBasis, focalForward) * sin(tilt);\n"
              "  const float denominator = dot(pinholeDirection, tiltedNormal);\n"
              "  const bool valid = abs(denominator) > 1.0e-7f;\n"
              "  float3 focalPoint = origin.xyz;\n"
              "  if (valid) {\n"
              "    const float numerator = parameters.primaryPathLensParameters.x *\n"
              "                            dot(focalForward, tiltedNormal);\n"
              "    focalPoint = origin.xyz +\n"
              "                 pinholeDirection * (numerator / denominator);\n"
              "  }\n"
              "  const float4 lensRight = lookAtMotion ?\n"
              "      float4(basis.right * length(parameters.primaryPathLensRight.xyz), 0.0f) :\n"
              "      parameters.primaryPathLensRight;\n"
              ...
              "  const float4 lensOrigin = origin + lensRight * lensSample.x +\n"
              "                            lensUp * lensSample.y;\n"
              "  const float3 rayDirection = valid ? normalize(focalPoint - lensOrigin.xyz) :\n"
              "                                      focalForward;\n"
              "  GpuDiffusePathStateRecord path;\n"
              "  path.ray.origin = lensOrigin;\n"
              "  path.ray.direction = float4(rayDirection, 0.0f);\n"
              ...
              "  }\n"
              "  if (parameters.primaryPathGenerationMode ==\n"
              "      gpuPrimaryPathGenerationModeOrthographic) {\n"
              "    return makeOrthographicPrimaryPath(parameters, pathIndex);\n"
              "  }\n"
              "  if (parameters.primaryPathGenerationMode ==\n"
              "      gpuPrimaryPathGenerationModeThinLens) {\n"
              "    return makeThinLensPrimaryPath(parameters, pathIndex);\n"
              "  }\n"
              "  if (parameters.primaryPathGenerationMode ==\n"
              "      gpuPrimaryPathGenerationModeEquirectangular) {\n"
              "    return makeEquirectangularPrimaryPath(parameters, pathIndex);\n"
              "  }\n"
              "  if (parameters.primaryPathGenerationMode ==\n"
              "      gpuPrimaryPathGenerationModeSpherical) {\n"
              "    return makeSphericalPrimaryPath(parameters, pathIndex);\n"
              "  }\n"
              "  if (parameters.primaryPathGenerationMode ==\n"
              "      gpuPrimaryPathGenerationModeFishEye) {\n"
              "    return makeFishEyePrimaryPath(parameters, pathIndex);\n"
              "  }\n"
              "  if (parameters.primaryPathGenerationMode ==\n"
              "      gpuPrimaryPathGenerationModeTiltShift) {\n"
              "    return makeTiltShiftPrimaryPath(parameters, pathIndex);\n"
              "  }\n"
              ...
              "  const float4 shiftedOrigin = float4(\n"
              "      ray.origin.xyz - transform.motionDelta.xyz * ray.timeSample, ray.origin.w);\n"
              "  result.origin = transformPoint(transform.inversePointMatrix0,\n"
              "                                 transform.inversePointMatrix1,\n"
              "                                 transform.inversePointMatrix2,\n"
              "                                 transform.inversePointMatrix3, shiftedOrigin);\n"
              ...
              "        }\n"
              "        LocalPrimitiveHit triangleHit = intersectTriangle(\n"
              "            primitiveRay, triangles[primitive.payloadOffset], ray.maxDistance);\n"
              "        if (hasTransform) {\n"
              "          triangleHit = transformHit(triangleHit, transform, ray.timeSample);\n"
              "        }\n"
              "        if (triangleHit.hit) {\n"
              ...
              "        }\n"
              "        LocalPrimitiveHit sphereHit = intersectSphere(\n"
              "            primitiveRay, spheres[primitive.payloadOffset], ray.maxDistance);\n"
              "        if (hasTransform) {\n"
              "          sphereHit = transformHit(sphereHit, transform, ray.timeSample);\n"
              "        }\n"
              "        if (sphereHit.hit) {\n"
              ...
              "        }\n"
              "        LocalPrimitiveHit planeHit = intersectPlane(\n"
              "            primitiveRay, planes[primitive.payloadOffset], ray.maxDistance);\n"
              "        if (hasTransform) {\n"
              "          planeHit = transformHit(planeHit, transform, ray.timeSample);\n"
              "        }\n"
              "        if (planeHit.hit) {\n"
              ...
              "        }\n"
              "        LocalPrimitiveHit rectangleHit = intersectRectangle(\n"
              "            primitiveRay, rectangles[primitive.payloadOffset], ray.maxDistance);\n"
              "        if (hasTransform) {\n"
              "          rectangleHit = transformHit(rectangleHit, transform, ray.timeSample);\n"
              "        }\n"
              "        if (rectangleHit.hit) {\n"
              ...
              "        }\n"
              "        LocalPrimitiveHit diskHit = intersectDisk(\n"
              "            primitiveRay, disks[primitive.payloadOffset], ray.maxDistance);\n"
              "        if (hasTransform) {\n"
              "          diskHit = transformHit(diskHit, transform, ray.timeSample);\n"
              "        }\n"
              "        if (diskHit.hit) {\n"
              ...
              "        }\n"
              "        LocalPrimitiveHit openCylinderHit = intersectOpenCylinder(\n"
              "            primitiveRay, openCylinders[primitive.payloadOffset], ray.maxDistance);\n"
              "        if (hasTransform) {\n"
              "          openCylinderHit = transformHit(openCylinderHit, transform, ray.timeSample);\n"
              "        }\n"
              "        if (openCylinderHit.hit) {\n"
              ...
              "        }\n"
              "        LocalPrimitiveHit torusHit = intersectTorus(\n"
              "            primitiveRay, tori[primitive.payloadOffset], ray.maxDistance);\n"
              "        if (hasTransform) {\n"
              "          torusHit = transformHit(torusHit, transform, ray.timeSample);\n"
              "        }\n"
              "        if (torusHit.hit) {\n"
    ...
    NSString* diffusePathLoopKernelSourceSuffix() {
      return @"GpuTracingMaterialRecord materialRecord(\n"
              ...
              "  material.portalOriginMatrix0 = float4(0.0f);\n"
              "  material.portalOriginMatrix1 = float4(0.0f);\n"
              "  material.portalOriginMatrix2 = float4(0.0f);\n"
              "  material.portalOriginMatrix3 = float4(0.0f);\n"
              "  material.portalDirectionMatrix0 = float4(0.0f);\n"
              "  material.portalDirectionMatrix1 = float4(0.0f);\n"
              "  material.portalDirectionMatrix2 = float4(0.0f);\n"
              ...
              "  const float endpointTolerance = max(pathLoopMinimumHitDistance,\n"
              "                                      lightDistance * 1.0e-5f);\n"
              "  return max(0.0f, lightDistance - endpointTolerance);\n"
              "}\n"
              "DirectLightEstimate directLightEstimate(\n"
              "    constant GpuDiffusePathLoopLaunchParameters& parameters,\n"
              "    device const uchar* sceneUpload,\n"
              "    const GpuIntersectionHitRecord hit,\n"
              "    const GpuDiffusePathStateRecord path,\n"
              ...
              "    if (light.valid == 0u || light.pdf <= 0.0f) {\n"
              "      continue;\n"
              "    }\n"
              ...
              "        shadowRayFor(point, light, path.ray.timeSample);\n"
              "    ++estimate.visibilityRayCount;\n"
              "    GpuIntersectionRay occlusionRay = visibilityRay;\n"
              "    occlusionRay.maxDistance = lightOcclusionMaxDistance(light.distance);\n"
              "    if (anySupportedHit(parameters, sceneUpload, occlusionRay)) {\n"
              ...
              "  return ray;\n"
              "}\n"
              "GpuDiffusePathStateRecord portalContinuationPath(\n"
              "    const GpuIntersectionHitRecord hit,\n"
              "    const GpuDiffusePathStateRecord path,\n"
              "    GpuTracingMaterialRecord material,\n"
              "    float4 accumulatedRadiance,\n"
              ...
              "  GpuDiffusePathStateRecord next = path;\n"
              "  next.accumulatedRadiance = accumulatedRadiance;\n"
              "  const float4 continuationThroughput = path.throughput * material.parameters;\n"
              ...
              "  }\n"
              "  const float3 incoming = normalize(path.ray.direction.xyz);\n"
              "  const float4 shiftedOrigin = float4(hit.point.xyz + incoming * pathLoopRayEpsilon,\n"
              "                                      1.0f);\n"
              "  const float3 direction = normalize(transformDirection(\n"
              "      material.portalDirectionMatrix0, material.portalDirectionMatrix1,\n"
              "      material.portalDirectionMatrix2, float4(incoming, 0.0f)).xyz);\n"
              "  next.ray = path.ray;\n"
              "  next.ray.origin = transformPoint(\n"
              "      material.portalOriginMatrix0, material.portalOriginMatrix1,\n"
              "      material.portalOriginMatrix2, material.portalOriginMatrix3, shiftedOrigin);\n"
              "  next.ray.direction = float4(direction, 0.0f);\n"
              ...
              "      if (materialSupported && material.kind == gpuTracingPortalMaterialKind) {\n"
              "        bool spawned = false;\n"
              "        next = portalContinuationPath(hit, path, material, path.accumulatedRadiance,\n"
              "                                      spawned);\n"
              "        step.continuationThroughput = spawned ? next.throughput : float4(0.0f);\n"
              "        step.flags = next.flags;\n"
              "      } else if (matteSupported) {\n"
              ...
              "          return step;\n"
              "        }\n"
              "        const DirectLightEstimate directLight = directLightEstimate(\n"
              ...
              "  const float d = 0.59f;\n"
              "  const float e = 0.14f;\n"
              "  const float y = (value * (a * value + b)) /\n"
              "                  (value * (c * value + d) + e);\n"
              "  return clamp(y, 0.0f, 1.0f);\n"
              "}\n"
              "float3 applyDisplayResolveTonemap(\n"
              "    constant GpuDiffusePathLoopLaunchParameters& parameters,\n"
              "    float3 color) {\n"
              "  if (parameters.displayResolveTonemap == gpuDisplayResolveTonemapReinhard) {\n"
              "    return float3(reinhardTonemapChannel(color.r),\n"
              "                  reinhardTonemapChannel(color.g),\n"
              "                  reinhardTonemapChannel(color.b));\n"
              "  }\n"
              "  if (parameters.displayResolveTonemap == gpuDisplayResolveTonemapAces) {\n"
              "    return float3(acesTonemapChannel(color.r), acesTonemapChannel(color.g),\n"
              ...
              "    return;\n"
              "  }\n"
              "  const float3 color = applyDisplayResolveTonemap(parameters,\n"
              "                                                  colorSum / float(sampleCount));\n"
    ...
    NSString* diffusePathLoopKernelSource() {
      return [diffusePathLoopKernelSourcePrefix()
        stringByAppendingString:[diffusePathLoopKernelSourcePrimaryPathDescriptors()
