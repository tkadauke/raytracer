    ...
    private:
      void draw(const detail::OpenGLRasterMesh& mesh, const Colord& background) {
        ...
        if (!program.addShaderFromSourceCode(QOpenGLShader::Vertex,
                                             ...
                                             "  fragmentMaterialSpecularColor = "
                                             "materialSpecularColor;\n"
                                             "  fragmentMaterialSpecularCoefficient = "
                                             "materialSpecularCoefficient;\n"
                                             "  fragmentMaterialSpecularExponent = "
        ...
        if (!program.addShaderFromSourceCode(
              ...
              "  return lit / samples;\n"
              "}\n"
              "void addDirectionalLighting(vec3 normal, vec3 viewDir, "
              "float shadow,\n"
              ...
              "sourceColor.g), sourceColor.b) * "
              "fragmentAlphaScale;\n"
              "  } else if (fragmentAlbedoMode > 3.5 && "
              "fragmentAlbedoMode < 4.5) {\n"
              "    float checker = mod(floor(fragmentWorldPosition.x) + "
              "floor(fragmentWorldPosition.z), 2.0);\n"
              "    vec3 checkerColor = mix(checkerBright, checkerDark, "
              "step(0.5, checker));\n"
              "    sourceColor = vec4(checkerColor, 1.0);\n"
              "    sourceColor.a = max(max(sourceColor.r, "
