...
namespace engine::wavefront {
  ...
  WavefrontRenderMetrics::BatchSummary::tracingExecutionCapabilities() const {
    ...
    const bool usesGpuStyleSampleStream = sampleStreamMode == "gpu_sample_stream";
    if (usesGpuStyleSampleStream) {
      records.sampling.gpuRng = render::TracingCapabilityRecord::restricted(
        ...
        "GPU sample stream dimensions are generated by the CPU reference path");
    } else {
      records.sampling.gpuRng = render::TracingCapabilityRecord::unsupported(
        Domain::Sampling, "sampling.gpu_rng", "GPU sample stream was not requested");
    }
    records.sampling.namedDimensions = render::TracingCapabilityRecord::cpu(
      Domain::Sampling, "sampling.named_dimensions",
      usesGpuStyleSampleStream ? "gpu_sample_stream" : "sampler");
