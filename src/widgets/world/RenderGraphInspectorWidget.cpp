...
struct RenderGraphInspectorWidget::Private {
  ...
  QString tracingCapabilityRestrictedSummary(const QJsonArray& capabilities) const;
...
QString RenderGraphInspectorWidget::Private::tracingCapabilityRestrictedSummary(
  const QJsonArray& capabilities) const {
  QStringList names;
  for (const QJsonValue& value : capabilities) {
    const QJsonObject capability = value.toObject();
    if (capability.value(QStringLiteral("support")).toString() != QStringLiteral("restricted"))
      continue;

    QString name = capabilityNameText(capability);
    const QString resolved =
      metadataIdentifierText(capability.value(QStringLiteral("resolvedDevice")).toString());
    const QString path =
      metadataIdentifierText(capability.value(QStringLiteral("executionPath")).toString());
    name += QStringLiteral(": %1").arg(resolved);
    if (!path.isEmpty())
      name += QStringLiteral(" via %1").arg(path);
...
void RenderGraphInspectorWidget::Private::addIntersectionBackendDetailRows(
  ...
  if (!tracingCapabilities.isEmpty()) {
    ...
    addDetailRow(rows, QStringLiteral("Restricted capabilities"),
                 tracingCapabilityRestrictedSummary(tracingCapabilities));
  ...
  addDetailIntegerMetadataRow(rows, QStringLiteral("Closest-hit query transfer bytes"), batching,
                              QStringLiteral("intersectionEstimatedClosestHitQueryTransferBytes"));
  addDetailIntegerMetadataRow(rows, QStringLiteral("Any-hit query transfer bytes"), batching,
                              QStringLiteral("intersectionEstimatedAnyHitQueryTransferBytes"));
  ...
  addDetailIntegerMetadataRow(rows, QStringLiteral("Resident path-loop primary sample chunk size"),
                              batching, QStringLiteral("residentPathLoopPrimarySampleChunkSize"));
  ...
  addDetailIntegerMetadataRow(rows, QStringLiteral("Compaction candidate host path-state bytes"),
                              batching,
                              QStringLiteral("frontierCompactionCandidateHostPathStateBytes"));
  ...
  addDetailIntegerMetadataRow(
    rows, QStringLiteral("Largest compaction candidate host path-state bytes"), batching,
    QStringLiteral("frontierLargestCompactionCandidateHostPathStateBytes"));
  ...
  addDetailIntegerMetadataRow(rows, QStringLiteral("Direct-light any-hit round trips"), batching,
                              QStringLiteral("directLightAnyHitQueryRoundTrips"));
  addDetailIntegerMetadataRow(rows, QStringLiteral("Resident direct-light round trips"), batching,
  ...
  addDetailIntegerMetadataRow(rows, QStringLiteral("Direct-light occlusion host bytes"), batching,
                              QStringLiteral("directLightOcclusionHostBytes"));
  ...
  addDetailStringMetadataRow(rows, QStringLiteral("Direct-light contribution execution"), batching,
                             QStringLiteral("directLightContributionExecutionPath"), true);
  addDetailStringMetadataRow(rows, QStringLiteral("Direct-light contribution fallback"), batching,
                             QStringLiteral("directLightContributionFallbackReason"));
  addDetailIntegerMetadataRow(rows,
...
QString RenderGraphInspectorWidget::Private::passTraceLine(const RenderPassNode& pass) const {
  ...
  if (!batching.isEmpty()) {
    ...
    const auto primarySampleChunkSize = static_cast<qulonglong>(
    ...
    if (!intersectionBackendRequest.isEmpty() || !intersectionBackend.isEmpty()) {
      ...
      if (batching.value(QStringLiteral("intersectionSceneCompiled")).toBool()) {
        ...
        if (compactionRemovedSamples > 0) {
          ...
          const QString compactionResidencyPath =
            batching.value(QStringLiteral("frontierCompactionPathStateResidency")).toString();
          const QString compactionResidency =
            compactionResidencyPath.isEmpty() ? QStringLiteral("host") : compactionResidencyPath;
          ...
          line +=
            QStringLiteral(
              ", %1 compaction on %2 path state removed %3 samples/%4 retained-index bytes/%5 "
              "removed host path-state bytes")
        ...
        if (compactionCandidateSamples > 0) {
          ...
          const qulonglong compactionCandidateHostPathStateBytes = jsonIntegerValue(
            batching, QStringLiteral("frontierCompactionCandidateHostPathStateBytes"));
          line += QStringLiteral(
                    ", %1 compaction candidate samples (%2 packed-ray bytes/%3 state bytes/%4 host "
                    "path-state bytes)")
                    .arg(compactionCandidateSamples)
                    .arg(compactionCandidatePackedBytes)
                    .arg(compactionCandidateStateBytes)
                    .arg(compactionCandidateHostPathStateBytes);
        }
        ...
        if (largestCompactionCandidateSamples > 0) {
          ...
          const qulonglong largestCompactionCandidatePackedBytes = jsonIntegerValue(
            batching, QStringLiteral("frontierLargestCompactionCandidatePackedRayBytes"));
          ...
          const qulonglong largestCompactionCandidateHostPathStateBytes = jsonIntegerValue(
            batching, QStringLiteral("frontierLargestCompactionCandidateHostPathStateBytes"));
          if (largestCompactionCandidateHostPathStateBytes > 0) {
            largestCompactionCandidateText += QStringLiteral(" (%1 host path-state bytes)")
                                                .arg(largestCompactionCandidateHostPathStateBytes);
          }
    ...
    if (directLightAnyHitBatchChunks > 0) {
      const qulonglong directLightOcclusionHostBytes =
        jsonIntegerValue(batching, QStringLiteral("directLightOcclusionHostBytes"));
      ...
      const qulonglong directLightAnyHitRoundTrips =
